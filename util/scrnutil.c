...
*/
	...
	if (data->save==YES)		/* Should the screen "underneath" be saved? */
		{
		data->blockptr=xmalloc( (data->x2-data->x1+1) * (data->y2-data->y1+1) * sizeof(int) );
...
void boxwrite(BOX *box, int x, int y, char far *line)
...

{
   ...
   scrptr = xcalloc(1, sizeof(SCRNLIST));

   scrptr->thisscreen = xmalloc(maxx*maxy*2);      /* Get the mem */
   scrptr->next = NULL;

   #ifndef __OS2__
   memcpy(scrptr->thisscreen, screen, maxx*maxy*2); /* Save it */
   #else
...

{
   ...
   #ifndef __OS2__
   memcpy(screen, temp->thisscreen, maxx*maxy*2); /* Restore it */
   #else
...
void saveblock(int x1, int y1, int x2, int y2, int * b)
...
void restoreblock(int x, int y, int w, int h, int *b)
{
   #ifndef __OS2__
	asm push ds
