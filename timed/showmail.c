...
dword ShowMsg(MMSG * msg, AREA * area, MSGA * areahandle, int displaytype)
{
    ...
    if (!(cfg.usr.status & CLOCK))
        sprintf(temp,
                "Area: %-31.31s 100%%  Current: %5ld, high: %5ld ³  %3dK ",
                area->desc, MsgGetCurMsg(areahandle), area->highest,
                (int)(((dword) coreleft()) / 1024));
    else
    {
        sprintf(temp, "Area: %-31.31s 100%%  Current: %5ld, high: %5ld ",
                area->desc, MsgGetCurMsg(areahandle), area->highest);
        update_clock(1);
    }
...
#ifdef __SENTER__
    if (numlines < maxy - 7)    /* if msg is shorter than screen, clear
                                   lower part of screen */
        ClsRectWith(6 + numlines, 0, maxy - 2, maxx - 1, cfg.col[Cmsgtext],
                    ' ');
#else
    if (numlines < maxy - 6)    /* if msg is shorter than screen, clear
                                   lower part of screen */
        ClsRectWith(5 + numlines, 0, maxy - 2, maxx - 1, cfg.col[Cmsgtext],
                    ' ');
#endif

    while (1)                   /* Exit is w/ return statements that gives 
                                   command back */
    {
        lptr = first;
        ...
        case cREADreturnbookmark: /* TAB */
            ...
            {
                if ((realno =
                     MsgUidToMsgn(areahandle, mark.number,
                                  UID_EXACT)) != 0)
                    return realno;
            }
            break;
        ...
        case cREADnext:        /* right */
            ...
            if (displaytype == SCAN_DISPLAY) /* No check for scanning
                                                functions */
                return NEXT;
            ...
            else
            {
                ...
                {
                    endarea =
                        initbox(11, 12, 17, 68, cfg.col[Cpopframe],
                                cfg.col[Cpoptext], SINGLE, YES, ' ');
                    drawbox(endarea);
                    boxwrite(endarea, 0, 1,
                             "           End of messages in this area!");
                    boxwrite(endarea, 2, 1,
                             "<right>, <enter> or '+' : next area with new mail");
                    boxwrite(endarea, 3, 1,
                             "<ESC>                   : exit area");
                    boxwrite(endarea, 4, 1,
                             "Other keys              : remove this text");
        ...
        case cREADgotooriginal: /* ctrl - left */
            ...
            realno =
                MsgUidToMsgn(areahandle, curmsg->mis.replyto, UID_EXACT);

            if ((realno != 0) &&
                ((msghandle =
                  MsgOpenMsg(areahandle, MOPEN_READ, realno)) != NULL))
            {
        ...
        case cREADnextreply:   /* ALT - right */
            ...
            realno =
                MsgUidToMsgn(areahandle, curmsg->mis.nextreply, UID_EXACT);
        ...
        case cREADdown:        /* down */
#ifdef __SENTER__
            if ((curline < numlines - (maxy - 7))
                && (numlines > (maxy - 7)))
#else
            if ((curline < numlines - (maxy - 6))
                && (numlines > (maxy - 6)))
#endif
                curline++;
            break;
        ...
        case cREADnextmarked:  /* ctrl-up */
            if (((curno = NextMarked(area->mlist, curmsg->uid)) == 0) ||
                ((curno =
                  MsgUidToMsgn(areahandle, curno, UID_EXACT)) == 0))
                beep();
            else
                return curno;
            break;
        ...
        case cREADwrite:       /* ALT - W */
            ...
            PrintMessage(curmsg, area, areahandle, 0, 0, PRINTALL); /* Last 
                                                                       element: 
                                                                       no
                                                                       hardcopy 
                                                                     */
        ...
        case cREADwritebody:
            ...
            PrintMessage(curmsg, area, areahandle, 0, 0, PRINTBODY); /* Last 
                                                                        element: 
                                                                        no 
                                                                        hardcopy 
                                                                      */
        ...
        case cREADwriterealbody:
            ...
            PrintMessage(curmsg, area, areahandle, 0, 0, PRINTREALBODY); /* Last 
                                                                            element: 
                                                                            no 
                                                                            hardcopy 
                                                                          */
        ...
        case cREADwriteheader:
            ...
            PrintMessage(curmsg, area, areahandle, 0, 0, PRINTHDR); /* Last 
                                                                       element: 
                                                                       no
                                                                       hardcopy 
                                                                     */
        ...
        case cREADprint:       /* ALT-P */
            ...
            PrintMessage(curmsg, area, areahandle, 1, 0, PRINTALL); /* Last 
                                                                       element: 
                                                                       a
                                                                       hardcopy 
                                                                     */
...
//               break;
        ...
        case 27:               // ESC, might be that ESC is not bound to
                                // a command..

            if (displaytype != SCAN_DISPLAY) // If not scanning, this is
                                             // only
        ...
        case cREADdelete:      /* ALT D */
                ...
                (confirm("Process marked messages? [y/N]") == 0))
            {
                if ((!(cfg.usr.status & CONFIRMDELETE)) ||
                    (confirm
                     ("Delete current message. Are you sure? (y/N)")))
                {
            ...
            else
            {
                if ((!(cfg.usr.status & CONFIRMDELETE)) ||
                    (confirm
                     ("Delete marked messages. Are you sure? (y/N)")))
            ...
            if (area->nomsgs == 0)
            {
                if (displaytype != SCAN_DISPLAY) // Don't return ESC in
                                                 // mailscan! It will
                                                 // abort it!
                    return ESC;
                else
        ...
        case cREADmove:        /* ALT - M */
            ...
            if (area->nomsgs == 0)
            {
                if (displaytype == SCAN_DISPLAY) // Don't return ESC in a
                                                 // mailscan!
                    return NEXT;
                else
                    return ESC;
        ...
        case cREADfreqfiles:   /* CTRL - F */
            ...
            if ((tempmsg =
                 MsgOpenMsg(areahandle, MOPEN_READ,
                            (int)MsgUidToMsgn(areahandle, this,
                                              UID_EXACT))) != NULL)
                MsgCloseMsg(tempmsg);
        ...
        case cREADmarkchain:   /* '*' */
            ...
            if ((tempmsg =
                 MsgOpenMsg(areahandle, MOPEN_READ,
                            (int)MsgUidToMsgn(areahandle, this,
                                              UID_EXACT))) != NULL)
                MsgCloseMsg(tempmsg); // reposition MSGAPI to current
                                      // msg..
            break;
        ...
        case cREADsearchcurmsg:
            ...
            if (retval != -1)   // something was found, try to center
                                // first found
            {
                curline = retval;
#ifdef __SENTER__
            ...
            if (curline > numlines - (maxy - 6)) /* Not entire screen
                                                    filled */
#ifdef __SENTER__
                ClsRectWith(6 + (numlines - curline - 1), 0, maxy - 2,
                            maxx - 1, cfg.col[Cmsgtext], ' ');
#else
                ClsRectWith(5 + (numlines - curline - 1), 0, maxy - 2,
                            maxx - 1, cfg.col[Cmsgtext], ' ');
#endif
...
dword getnumber(char c, MSGA * areahandle, AREA * area)
{
    ...
    curno = MsgGetCurMsg(areahandle); // Save position, to later reset
                                      // MSGAPI.
    ...
    numbox =
        initbox(12, 18, 14, 41, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(numbox);
    ...
    while ((ret != ESC) && (ret != 0))
        ret =
            getstring(13, 35, number, 6, 6, "0123456789", cfg.col[Centry],
                      cfg.col[Cpoptext]);
    ...
    if ((area->type != ECHOMAIL && area->type != NEWS) &&
        (cfg.usr.status & RESPECTPRIVATE) && !MaySee(areahandle, n))
    {
...
char *MakeRep(MMSG * msg, MSGA * areahandle, AREA * area)
{
    ...
    while ((n < 9) && (msg->mis.replies[n] != 0))
    {
        ...
        if (realno && (!(BePrivate(area) && !MaySee(areahandle, realno))))
        {
            gotone = 1;
    ...
    if (gotone)
        repstring[5] = 16;      /* This is a  */

    if (MsgGetCurMsg(areahandle) != wherenow)
    {
        if ((msghandle =
             MsgOpenMsg(areahandle, MOPEN_READ, wherenow)) != NULL)
...
dword PickReply(MMSG * curmsg, MSGA * areahandle)
{
    ...
    {
        realno =
            MsgUidToMsgn(areahandle, curmsg->mis.replies[0], UID_EXACT);
    ...
    names = mem_calloc(11, sizeof(char *)); /* Alloc space for 11 names,
                                               10 is max for squish */
    while (n < 9)
    {
        if ((realno =
             MsgUidToMsgn(areahandle, curmsg->mis.replies[n],
                          UID_EXACT)) != 0)
        {
            if ((msghandle =
                 MsgOpenMsg(areahandle, MOPEN_READ, realno)) != NULL)
            {
                if (MsgReadMsg(msghandle, &mis, 0, 0, NULL, 0, NULL) != -1)
                {
                    ...
                    reps[tot] = realno;
                    sprintf(names[tot++], " %-4.4lu %-34s ", realno,
                            mis.from);
                }
                MsgCloseMsg(msghandle);
            }
...
int HighlightLines(MMSG * curmsg)
{
    ...
    inputbox =
        initbox(10, 10, 15, 70, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(inputbox);
    boxwrite(inputbox, 0, 1, "Give the keyword to look for:");
    ret =
        getstring(13, 12, keyword, 56, 79, "", cfg.col[Centry],
                  cfg.col[Cpoptext]);
    delbox(inputbox);
