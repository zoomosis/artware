...
int shell_to_DOS(void)
{
    ...
    undo09();
    retval =
        do_exec("", "", USE_ALL | HIDE_FILE | CHECK_NET,
                (cfg.usr.status & SWAPSHELL) ? 0xFFFF : 0x0000, environ);
    ins09();
#else
                ...
                long lastorhigh, dword highest)
{
    ...
    undo09();
    retval =
        do_exec(prog, parms, USE_ALL | HIDE_FILE | CHECK_NET,
                (cfg.usr.status & SWAPSHELL) ? 0xFFFF : 0x0000, environ);
    ins09();
#else
    {
                   ...
                   MSGA * areahandle, MMSG * curmsg)
{
    ...
    inputbox =
        initbox(10, 0, 15, 79, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(inputbox);
    boxwrite(inputbox, 0, 1,
             "Give the name of program & the parameters to use:");
    ...
    {
        ret =
            getstring(13, 11, prog, 67, 69, "", cfg.col[Centry],
                      cfg.col[Cpoptext]);
    }
    while (ret != ESC && ret != 0);
    ...
    {
        ret =
            getstring(14, 11, parms, 67, 69, "", cfg.col[Centry],
                      cfg.col[Cpoptext]);
    }
    while (ret != ESC && ret != 0);
    ...
    *parmsptr =
        BuildCommandLine(parms, area, areahandle, curmsg, NULL, NULL);
                       ...
                       MMSG * curmsg, char *curfile, char *newfile)
{
    ...
    {
        if (*charptr != '[' || *(charptr + 1) == '[') // No 'variable' or
                                                      // two [[
            *tmpptr++ = *charptr++;
        else
        {
            ...
            else if (strncmpi(charptr + 1, "format", 6) == 0)
            {
                if (area->base & MSGTYPE_JAM)
                    ...
                    strcpy(temp, "hmb");
                else if (area->base & MSGTYPE_SQUISH)
                    strcpy(temp, "squish");
                charptr += 7;
            }
            ...
            if (*charptr == ']')
                charptr++;      // If it ends with ], skip it.
        }
    }
...
int FileDelete(void)
{
    ...
    inputbox =
        initbox(10, 2, 15, 77, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(inputbox);
    boxwrite(inputbox, 0, 1, "Give the name of the file to delete:");
    ret =
        getstring(13, 4, filename, 72, 79, "", cfg.col[Centry],
                  cfg.col[Cpoptext]);
    delbox(inputbox);
