...
sword ReadRawNodeList(sword zone, sword net, sword node, sword point,
                      long offset, ADDRLIST * found);
...
int FDinit(int syslookup)
{
    ...
    if (syslookup)
    {
        if (FDinit(0) == -1)    // To get the name (extention) of list,
                                // only in node index..
            return -1;
        close(fdidx);
...
short int FDcurr(ADDRLIST * found)
{
    ...
    if (curaction == CURACTIONNODE)
    {
        if (ReadRawNodeList(swaphilo(fdnnode.nodedata[GetCurEntry()].zone),
                            ...
                            swaphilo(fdnnode.nodedata[GetCurEntry()].
                                     point),
    ...
    else
    {
        if (ReadRawNodeList(swaphilo(fdunode.nodedata[GetCurEntry()].zone),
                            ...
                            swaphilo(fdunode.nodedata[GetCurEntry()].
                                     point),
...
short int FDhead(ADDRLIST * found)
{
    ...
    if (curaction == CURACTIONNAME)
    {
        ...
        while (fdunode.nextlevel != 0) // Go back in index, until we hit a 
                                       // leaf
        {
            SetCurEntry(-1);
            if (DiveInto(fdunode.nextlevel, &fdunode, sizeof(FDUNODE)) ==
                -1)
                return ERROR;
        }
    ...
    else                        // Node lookup
    {
        ...
        while (fdnnode.nextlevel != 0) // Go back in index, until we hit a 
                                       // leaf
        {
            SetCurEntry(-1);
            if (DiveInto(fdnnode.nextlevel, &fdnnode, sizeof(FDNNODE)) ==
                -1)
                return ERROR;
        }
...
short int FDtail(ADDRLIST * found)
{
    ...
    if (curaction == CURACTIONNAME)
    {
        ...
        while (fdunode.nextlevel != 0) // Go fwd in index, until we hit a
                                       // leaf
        {
            SetCurEntry(fdunode.count - 1);
            if (DiveInto
                (fdunode.nodedata[fdunode.count - 1].ptr, &fdunode,
                 sizeof(FDUNODE)) == -1)
                return ERROR;
        }
    ...
    else                        // Node lookup
    {
        ...
        while (fdnnode.nextlevel != 0) // Go fwd in index, until we hit a
                                       // leaf
        {
            SetCurEntry(fdnnode.count - 1);
            if (DiveInto
                (fdnnode.nodedata[fdnnode.count - 1].ptr, &fdnnode,
                 sizeof(FDNNODE)) == -1)
                return ERROR;
        }
...
short int FDfindmark(ADDRLIST * found)
{
    ...
    {
        if (get_node
            (sstack[sstacksize - 1].node, &fdunode, sizeof(FDUNODE)) == -1)
            return ERROR;
    }
    else
    {
        if (get_node
            (sstack[sstacksize - 1].node, &fdnnode, sizeof(FDNNODE)) == -1)
            return ERROR;
    }
            ...
            if (k == 0)         /* match! */
            {
                if (found)
                    ReadRawNodeList(swaphilo(fdunode.nodedata[j].zone),
                                    ...
                                    fdunode.nodedata[j].offset, found);
                return FOUND;
            }
        }
            ...
            if (k == 0)         /* match! */
            {
                if (found)
                    ReadRawNodeList(swaphilo(fdnnode.nodedata[j].zone),
                                    ...
                                    fdnnode.nodedata[j].offset, found);
                return FOUND;
            }
        }
...
int FDnextname(void)
{
    ...
    {
        if (GetCurEntry() < (fdunode.count - 1)) // Can we get next item
                                                 // from this leaf?
        {
        ...
        do
        {
            if ((status = BackupFromFDU(&fdunode)) != OK) // Go back into
                                                          // index node.
                return status;
    ...
    else                        // We are in an index node.
    {
        do
        {
            ...
            {
                if (DiveInto
                    (fdunode.nodedata[GetCurEntry()].ptr, &fdunode,
                     sizeof(FDUNODE)) != 0)
                    return ERROR;
            }
...
int FDnextnode(void)
{
    ...
    {
        if (GetCurEntry() < (fdnnode.count - 1)) // Can we get next item
                                                 // from this leaf?
        {
            SetCurEntry(GetCurEntry() + 1);
            return OK;
        ...
        do
        {
            if ((status = BackupFromFDN(&fdnnode)) != OK) // Go back into
                                                          // index node.
                return status;
    ...
    else                        // We are in an index node.
    {
        do
        {
            ...
            {
                if (DiveInto
                    (fdnnode.nodedata[GetCurEntry()].ptr, &fdnnode,
                     sizeof(FDNNODE)) != 0)
                    return ERROR;
            }
...
int FDprevname(void)
{
    ...
    {
        SetCurEntry(GetCurEntry() - 1);
        do                      // Dive into previous link, all the way to 
                                // the leaf node.
        {
            if (GetCurEntry() == -1)
            {
                if (DiveInto(fdunode.nextlevel, &fdunode, sizeof(FDUNODE)) != 0) // special 
                                                                                 // case, 
                                                                                 // nxtlvl
                    return ERROR;
            }
...
int FDprevnode(void)
{
    ...
    else                        // We are in an index node.
    {
                                ...
                                // the leaf node.
        {
            if (GetCurEntry() == -1)
            {
                if (DiveInto(fdnnode.nextlevel, &fdnnode, sizeof(FDNNODE)) != 0) // special 
                                                                                 // case, 
                                                                                 // nxtlvl
                    return ERROR;
            }
...
void CopyTillComma(char **in, char *out, int maxlen)
{
    ...
    *out = '\0';
    // Maybe string is longer than maxlen chars, skip to comma then
    // anyway.
    ...
    *in = inptr;                // set original pointer, so calling f'ion
                                // knows where he is
}
