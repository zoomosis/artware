...
void SaveTags(void)
{
    ...
    if (choice >= 0 && choice <= 9)
    {
        ...
        savebox =
            initbox(maxy / 2 - 2, maxx / 2 - 30, maxy / 2 + 2,
                    maxx / 2 + 30, cfg.col[Cpopframe], cfg.col[Cpoptext],
                    SINGLE, YES, ' ');

        drawbox(savebox);
        titlewin(savebox, TLEFT, " Description", 0);
        ret =
            getstring(maxy / 2, maxx / 2 - 28, desc, 55, 55, "",
...
char *GetDesc(int no)
{
    ...
    if (fgets(line, sizeof line, in) != NULL)
    {
        if (line[0] == ';')     // description?
            ...
            strcpy(desc, "No description present."); // Better than
                                                     // nothing!
    }
