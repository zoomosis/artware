...
static int idxin,
           datin,
           idxout,
           datout;  // File handles for input/output
...
int main(void)
{
   ...
   while(GetNextIdx() != -1)
     {
     if(ReadBase() == -1) continue;  // Deleted record.
     printf("Area: %-15.15s\r", diskbase.thisareatag); fflush(stdout);
     WriteIdx();
     WriteBase();
...
int ReadBase(void)
{
  ...
  if(read(datin, &diskbase, (unsigned) sizeof(DISKBASE)) != (int) sizeof(DISKBASE))
    {
  ...
  if(diskbase.deleted == 1) return -1;
...
void WriteIdx(void)
{
   ...
   if(write(idxout, &findidx, (unsigned) sizeof(FINDIDX)) != (int) sizeof(FINDIDX))
     {
...
void WriteBase(void)
{
  ...
  if(write(datout, &diskbase, (unsigned) sizeof(DISKBASE)) != (int) sizeof(DISKBASE))
    {
...
void CopyRest(void)
{
   ...
   do
     {
     if(read(datin, &diskarg, (unsigned) sizeof(DISKARG)) != (int) sizeof(DISKARG))
       {
       puts("Error reading find database!\n");
       exit(254);
       }

     if(write(datout, &diskarg, (unsigned) sizeof(DISKARG)) != (int) sizeof(DISKARG))
       {
       puts("Error writing find database!\n");
       exit(254);
       }
...
void OpenFindDatabase(void)
{
   if((idxin = sopen("fdb.idx", O_BINARY | O_RDONLY, SH_DENYRW, S_IREAD|S_IWRITE)) == -1)
     goto error;

   if((datin = sopen("fdb.dat", O_BINARY | O_RDONLY, SH_DENYRW, S_IREAD|S_IWRITE)) == -1)
     goto error;

   if((idxout = sopen("$fdb.idx", O_BINARY | O_CREAT | O_TRUNC | O_WRONLY, SH_DENYWR, S_IREAD|S_IWRITE)) == -1)
     goto error;

   if((datout = sopen("$fdb.dat", O_BINARY | O_CREAT | O_TRUNC | O_WRONLY, SH_DENYWR, S_IREAD|S_IWRITE)) == -1)
     goto error;
