               ...
               UMSGID reply_to_id, char *subject, MMSG * newmmsg)
{
    ...
    if (reply == 2)             // Shut off Usenet stuff for
                                // OrigReply/Followup
    {
        curmsg->status &= ~REPLYTO;
        // If the original is addressed to internet, do it for this as
        // well.
    ...
    if (reply)
    {
        if ((!(curmsg->status & REPLYTO)) ||
            ...
            strcpy(newmmsg->mis.to,
                   (reply == 2) ? curmsg->mis.to : curmsg->mis.from);
        else
        {
        ...
        strcpy(newmmsg->mis.subj,
               strip_subject(curmsg->mis.subj, area->type == NETMAIL));
    ...
    if (reply)
    {
        if (!(curmsg->status & REPLYTO))
        {
            newmmsg->mis.destfido =
                (reply == 2) ? curmsg->mis.destfido : curmsg->mis.origfido;
        }
               ...
               int domatch, AREA * area)
{
    ...
    while (what < 7)
    {
        ...
        case 1:
            bottom
                ("Type your name or alias, press ~TAB~ to get selection list..");
            ret = getstring(1, 13, curmsg->mis.from, 65, 99, "", cfg.col[Centry], cfg.col[Cfinddata]); /* Edit 
                                                                                                          the 
                                                                                                          FROM: 
                                                                                                          field 
                                                                                                        */
        ...
        case 2:
            ...
            bottom
                ("Type your address, press ~TAB~ to get selection list..");
            strcpy(tempaddress, FormAddress(&curmsg->mis.origfido));
            strcpy(addrcopy, tempaddress);
            ret =
                getstring(2, 13, tempaddress, 29, 79, ADDRESSCHARS,
                          cfg.col[Centry], cfg.col[Cfinddata]);
            if (strcmp(addrcopy, tempaddress) != 0) /* Did address change? 
                                                     */
                akamatched = 1; /* Never match AKA after manual change */

            if (tempaddress[0] != '\0')
                address_expand(tempaddress, &curmsg->mis.origfido, aka);
            ...
            if (ret == TAB)
            {
                if ((ret = choose_address()) != -1)
                {
                    ...
                    ret = 0;
                    akamatched = 1; /* Never match on manually changed
                                       address */
                }
                else
                    ret = ESC;
        ...
        case 3:
            if (address)
                bottom
                    ("Name or address to send to, ~F2~ for a fresh nodelist lookup, ~TAB~ for a macrolist");
            else
                bottom
                    ("Type name of the person to send this message to..");

            strcpy(tempaddress, curmsg->mis.to); /* Save copy, to detect a 
                                                    change */

            if (address && curmsg->mis.destinter[0] != '\0')
            {
                ret = getstring(3, 13, curmsg->mis.destinter, 65, 99, "", cfg.col[Centry], cfg.col[Cfinddata]); /* Edit 
                                                                                                                   the 
                                                                                                                   TO: 
                                                                                                                   field 
                                                                                                                 */
                if (strchr(curmsg->mis.destinter, '@') == NULL && strchr(curmsg->mis.destinter, '!') == NULL) // I'net 
                                                                                                              // address 
                                                                                                              // lost..
                {
                    strcpy(curmsg->mis.to, curmsg->mis.destinter);
                    memset(curmsg->mis.destinter, '\0',
                           sizeof(curmsg->mis.destinter));
                }
            }
            else
            {
                ret = getstring(3, 13, curmsg->mis.to, 65, 99, "", cfg.col[Centry], cfg.col[Cfinddata]); /* Edit 
                                                                                                            the 
                                                                                                            TO: 
                                                                                                            field 
                                                                                                          */
            ...
            if ((ret != 0) &&
                (ret != F2) &&
                (ret != RESET) &&
                (ret != ATTACH) && (ret != SAVE) && (ret != TAB))
                break;          /* <ENTER> not pressed */
            ...
            if ((ret != F2) && ((alias = check_alias(&curmsg->mis)) != 0)) /* Is 
                                                                              it 
                                                                              an 
                                                                              alias? 
                                                                            */
            {
                ...
                if (alias == 2) /* Also filled in subject, do AKA matching 
                                   now! */
                {
            ...
            if (address)        /* Netmail, we need to do nodelist
                                   lookups.. */
            {
                // Let's first check to see if an internet address is
                // filled
                // in. In that case we need to move the internet address
                // to
...
//                      found = getFDnode(&tempnet);
                ...
                else if (check_node(&curmsg->mis, aka, PROMPT) == 1) /* Found 
                                                                        it? 
                                                                      */
                {
        ...
        case 4:
            ...
            if ((curmsg->mis.destfido.zone == 0)
                && (curmsg->mis.destfido.net == 0)
                && (curmsg->mis.destfido.node == 0)
                && (curmsg->mis.destfido.point == 0))
                memset(tempaddress, '\0', sizeof(tempaddress));
            ret =
                getstring(4, 13, tempaddress, 29, 99, ADDRESSCHARS,
                          cfg.col[Centry], cfg.col[Cfinddata]);
            if (tempaddress[0] != '\0')
                address_expand(tempaddress, &curmsg->mis.destfido, aka);
        ...
        case 5:
            ...
            if ((!akamatched) && (address))
            {
                if (matchaka(&curmsg->mis) != 0)
                {
                    PaintNewHeader(curmsg, areahandle->type,
                                   (pAKA | pUPDATE));
                }
            ...
            {
                ret = 0;        // Skip subject for attach/request in
                                // non-JAM
                break;
            }

            bottom("Give the subject of the message..");
            ret = getstring(5, 13, curmsg->mis.subj, 65, 99, "", cfg.col[Centry], cfg.col[Cfinddata]); /* Edit 
                                                                                                          the 
                                                                                                          SUBJ: 
                                                                                                          field 
                                                                                                        */
        ...
        case BTAB:
            ...
            if ((what == 5) &&
                ...
                (curmsg->mis.attached || curmsg->mis.requested))
                what--;         // Skip subj. - non-JAM attach/request
            break;
        case ESC:
            return ABORT;
        default:
        ...
        {
            if ((curmsg->mis.attr1 & aFILE)
                && curmsg->mis.attached == NULL)
                Message("Warning: attach bit set, but no files attached!",
                        -1, 0, YES);

            if ((curmsg->mis.attr1 & aFRQ)
                && curmsg->mis.requested == NULL)
                Message
                    ("Warning: request bit set, but no files requested!",
                     -1, 0, YES);
...
int SetAttributes(MMSG * curmsg, word base, int fullscreen)
{
    ...
    {
        attrbox =
            initbox(8, 16, 22, 63, cfg.col[Casframe], cfg.col[Castext],
                    SINGLE, YES, ' ');
        drawbox(attrbox);
        ...
        biprint(10, 17, cfg.col[Castext], cfg.col[Casaccent], " ~C~rash",
                '~');
        biprint(11, 17, cfg.col[Castext], cfg.col[Casaccent],
                " ~A~ttach file", '~');
        biprint(12, 17, cfg.col[Castext], cfg.col[Casaccent],
                " ~R~equest file", '~');
        biprint(13, 17, cfg.col[Castext], cfg.col[Casaccent],
                " ~K~ill/sent", '~');
        ...
        biprint(18, 17, cfg.col[Castext], cfg.col[Casaccent], " ~S~ent",
                '~');
        biprint(19, 17, cfg.col[Castext], cfg.col[Casaccent],
                " recei~V~ed", '~');
        biprint(20, 17, cfg.col[Castext], cfg.col[Casaccent], " ~D~irect",
                '~');
        biprint(21, 17, cfg.col[Castext], cfg.col[Casaccent],
                " ~U~pdate req.", '~');

        biprint(9, 36, cfg.col[Castext], cfg.col[Casaccent], "~O~rphan",
                '~');
        biprint(11, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~Y~ return receipt", '~');
        biprint(10, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~Q~ ret. receipt re[Q]uest", '~');

        print(13, 36, cfg.col[Castext], "Extra 'flags':");

        biprint(15, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~E~rase file when sent", '~');
        biprint(16, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~X~ archive file when sent", '~');
        biprint(17, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~I~mmediate", '~');
        biprint(18, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~T~runcate file when sent", '~');
        biprint(19, 36, cfg.col[Castext], cfg.col[Casaccent], "~W~ Lock",
                '~');
        biprint(20, 36, cfg.col[Castext], cfg.col[Casaccent],
                "~M~ confirm receipt request", '~');
    }
    else
    {
#ifdef __SENTER__
        attrbox =
            initbox(8, 33, 15, 60, cfg.col[Casframe], cfg.col[Castext],
                    SINGLE, NO, ' ');
#else
        attrbox =
            initbox(8, 33, 14, 44, cfg.col[Casframe], cfg.col[Castext],
                    SINGLE, NO, ' ');
#endif
        drawbox(attrbox);
        ...
        biprint(12, 34, cfg.col[Castext], cfg.col[Casaccent], " ~S~ent",
                '~');
        biprint(13, 34, cfg.col[Castext], cfg.col[Casaccent],
                " recei~V~ed", '~');
#ifdef __SENTER__
        biprint(14, 34, cfg.col[Castext], cfg.col[Casaccent],
                " Confir~M~ receipt request", '~');
#endif
    }
    ...
    if (fullscreen)
        ...
        statusbar
            ("Press letter to toggle attributes, ~ENTER~ to continue..");

    while (1)
    {
        if (!fullscreen)
#ifdef __SENTER__
            vprint(1, 40, cfg.col[Cmsgattribs], "%40.40s",
                   attr_to_txt(mis->attr1, mis->attr2));
#else
            vprint(0, 40, cfg.col[Cmsgattribs], "%40.40s",
                   attr_to_txt(mis->attr1, mis->attr2));
#endif
        else
            PaintNewHeader(curmsg, base, pUPDATE); // local, who cares
                                                   // (just attribs)
...
int matchaka(MIS * mis)
{
    ...
    if ((thisone = cfg.akaforce) != NULL)
    {
        while (thisone)
        {
            ...
            {
                if (addrcmp(&cfg.usr.address[thisone->aka], &mis->origfido)
                    == 0)
    ...
    if ((cfg.usr.status & AKAMATCHING) == 0)
        return 0;               /* AKA matching is off.. */
    ...
    if (akano == 1)
        return 0;               /* Only 1 AKA, go home! */

    for (hits = 0, i = 0; i < akano; i++) /* Check for zone-match for all
                                             AKA's */
    {
    ...
    if (hits > 1)
    {
        for (hits = 0, i = 0; i < akano; i++) /* Check for zone AND net */
        {
                ...
                (cfg.usr.address[i].net == mis->destfido.net))
            {
                ...
                if (found_zone_net == 0)
                {
                    firsthit = i; /* Found a match, use it, even if there
                                     would be more */
                    found_zone_net = 1;
                }
            }                   /* if */
    ...
    if (currentmatcheszone && !found_zone_net) /* The current address is
                                                  one of the matches, no
                                                  change */
        return 0;
...
int near MaskMatch(NETADDR * adr, NETADDR * mask)
{
    if ((mask->zone != (word) - 99) && (adr->zone != mask->zone))
        return 0;

    if ((mask->net != (word) - 99) && (adr->net != mask->net))
        return 0;

    if ((mask->node != (word) - 99) && (adr->node != mask->node))
        return 0;

    if ((mask->point != (word) - 99) && (adr->point != mask->point))
        return 0;
...
int aliaslist(MIS * mis)
{
    ...
    for (thismacro = cfg.firstmacro; thismacro;
    ...
    for (thismacro = cfg.firstmacro, i = 0; thismacro;
         thismacro = thismacro->next, i++)
    {
        if (thismacro->usenet[0] != '\0')
            sprintf(temp, " %-8.8s - %-39.39s", thismacro->macro,
                    thismacro->usenet);
        else
            sprintf(temp, " %-8.8s - %-39.39s", thismacro->macro,
                    thismacro->toname);
    ...
    {
        for (thismacro = cfg.firstmacro, i = 0; i < ret;
             thismacro = thismacro->next, i++)
            /* nothing */ ;
...
void PaintNewHeader(MMSG * curmsg, word type, word special)
{
    ...
    {
        if (special & pAKA)
            vprint(2, 13, cfg.col[Cfinddata], "%-51.51s (AKA matched)",
                   FormAddress(&mis->origfido));
        else
            vprint(2, 13, cfg.col[Cfinddata], "%-65.65s",
                   FormAddress(&mis->origfido));
    }
    ...
    if (!(type & MSGTYPE_NEWS))
    {
        ...
        {
            if (special & pUNKNOWN)
                vprint(4, 13, cfg.col[Cfinddata],
                       "%-47.47s (Unknown address)",
                       FormAddress(&mis->destfido));
            else
                vprint(4, 13, cfg.col[Cfinddata], "%-65.65s",
                       FormAddress(&mis->destfido));
        }
    }
    ...
    vprint(6, 13, cfg.col[Cfinddata], "%-65.65s",
           attr_to_txt(mis->attr1, mis->attr2));
...
void PaintHeaderFrame(void)
{
    ...
    box =
        initbox(0, 0, maxy - 2, 79, cfg.col[Cfindpassive],
                cfg.col[Cfindtext], SINGLE, NO, ' ');
    drawbox(box);
    delbox(box);

    box =
        initbox(7, 38, maxy - 4, 62, cfg.col[Cfindpassive],
                cfg.col[Cfindtext], SINGLE, NO, ' ');
    drawbox(box);
    delbox(box);

    printc(0, 11, cfg.col[Cfindpassive], 'Ñ');
    biprint(1, 1, cfg.col[Cfindtext], cfg.col[Cfindpassive],
            "Sender    ~³~", '~');
    biprint(2, 1, cfg.col[Cfindtext], cfg.col[Cfindpassive],
            "ÀÄ Address~³~", '~');
    biprint(3, 1, cfg.col[Cfindtext], cfg.col[Cfindpassive],
            "Recipient ~³~", '~');
    biprint(4, 1, cfg.col[Cfindtext], cfg.col[Cfindpassive],
            "ÀÄ Address~³~", '~');
    biprint(5, 1, cfg.col[Cfindtext], cfg.col[Cfindpassive],
            "Subject   ~³~", '~');
    biprint(6, 1, cfg.col[Cfindtext], cfg.col[Cfindpassive],
            "Attributes~³~", '~');

    printn(7, 0, cfg.col[Cfindpassive],
           "ÃÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´",
           80);
    printn(8, 2, cfg.col[Cfindtext], "Attached files:", 15);
    printn(8, 40, cfg.col[Cfindtext], "Requested files:", 16);
    biprint(9, 0, cfg.col[Cfindpassive], cfg.col[Cfindtext],
            "ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´ ~Name         Password~ ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´",
            '~');
    ...
    printn(maxy - 4, 0, cfg.col[Cfindpassive],
           "ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´",
           80);
    biprint(maxy - 3, 2, cfg.col[Cfindtext], cfg.col[Cfindaccent],
            "~ALT-A~: Attach files                   ~ALT-R~: Request files    ~ALT-S~: Special",
            '~');
...
void ShowAttached(MIS * mis)
{
    ...
    for (current = alist, i = 0; current && i < (maxy - 5 - 9);
         current = current->next, i++)
    {
        vprint(10 + i, 2, cfg.col[Cfindtext], "%-27.27s %4lu kB",
...
void ShowRequested(MIS * mis)
{
    ...
    for (current = mis->requested, i = 0; current && i < (maxy - 5 - 10);
         current = current->next, i++)
    {
...
void EditRequest(MMSG * curmsg, word type)
{
    ...
    if (!(type & MSGTYPE_NET))
    {
        Message("Requests can only be entered in netmail areas!", -1, 0,
                YES);
        return;
    }
    ...
    {
        Message("Simultaneous attach & request only in JAM areas!", -1, 0,
                YES);
        return;
    }
  ...
  start:
    ...
    for (current = mis->requested, howmany = 0; current;
    ...
    {
        bottom
            (" ~INS~: add request   ~DEL~: Delete request   ~ctrl-enter~: Accept");

        if (howmany > 0)
        {
            // find first to display
            for (i = 0, current = mis->requested; i < start;
            ...
            for (i = 0; i < rows; i++, current = current->next)
            {
                ...
                vprint(11 + i, 40, colour, "%-12.12s %-8.8s", current->s,
                       current->pw);
            }
        }
...
//             break;
        ...
        case 13:
            ...
            for (i = 0; i < curline; i++)
            {
                ...
                {
                    Message
                        ("Attempt to edit non-existant element from stringlist!",
                         -1, 0, YES);
                    break;
                }
                current = current->next;
        ...
        case 339:              /* Delete */
            ...
            if (howmany >= (maxy - 5 - 9)) // If more than one screen
                                           // full..
            {
                if (start && (start > (howmany - maxy - 5 - 9))) // Last
                                                                 // line
                                                                 // still
                                                                 // filled?
                    start--;
...
void EditAttach(MMSG * curmsg, word type)
{
    ...
    {
        Message("Attaches can only be entered in netmail areas!", -1, 0,
                YES);
        return;
    }
  ...
  start:
    ...
    {
        bottom
            (" ~Ins~: Add attach   ~Del~: Delete attach   ~ctrl-enter~: Accept");

        if (howmany > 0)
        {
            // find first to display
            for (i = 0, current = alist; i < start;
                 i++, current = current->next)
                /* nothing */ ;
            ...
            for (i = 0; i < rows; i++, current = current->next)
            {
                ...
                vprint(10 + i, 2, colour, "%-27.27s %4lu kB",
                       current->name, current->size);
            }
        }
...
//             break;
        ...
        case 338:              /* Insert */
            getnamebox =
                initbox(9, 2, 14, 77, cfg.col[Cfindactive],
                        cfg.col[Cfindtext], SINGLE, YES, ' ');
            drawbox(getnamebox);
            ...
            {
                retval =
                    getstring(12, 4, mask, 72, 71, VFNWW, cfg.col[Centry],
                              cfg.col[Cfindtext]);
            }
        ...
        case 13:
            ...
            if (retval != ESC && mask[0] != '\0')
            {
                ...
                strcpy(highlighted->name,
                       strlen(mask) < 28 ? mask : CompressPath(mask));
...
void RemoveElement(STRINGLIST ** start, int elno)
{
    ...
    for (i = 0; i < elno; i++)
    {
        ...
        {
            Message
                ("Attempt to remove non-existant element from stringlist!",
                 -1, 0, YES);
            return;
        }
        prev = current;
...
void SetSpecial(MMSG * curmsg)
{
    ...
    {
        printn(10, 68,
               whatnow == 0 ? cfg.col[Cfindhigh] : cfg.col[Cfindtext],
               "Encrypt   ", 10);
        printn(11, 68,
               whatnow == 1 ? cfg.col[Cfindhigh] : cfg.col[Cfindtext],
               "Sign      ", 10);
        printn(12, 68,
               whatnow == 2 ? cfg.col[Cfindhigh] : cfg.col[Cfindtext],
               "Spellcheck", 10);
...
int EditOneRequest(char *filename, char *password)
{
    ...
    getnamebox =
        initbox(10, 5, 15, 75, cfg.col[Cfindactive], cfg.col[Cfindtext],
                SINGLE, YES, ' ');
    drawbox(getnamebox);
    ...
    {
        retval =
            getstring(12, 28, filename, 39, 39, VFNWW, cfg.col[Centry],
                      cfg.col[Cfindtext]);
    }
    while (retval != 0 && retval != ESC);
    ...
    {
        retval =
            getstring(13, 28, password, 19, 19, VFNWW, cfg.col[Centry],
                      cfg.col[Cfindtext]);
    }
