...
int ReadKeyFile(void)
{
   ...
   while((temp=xgetline(keyfile)) != NULL)
     ...
     if(strcmpi(key, "386") == 0 ||
        ...
        key=strtok(NULL, " \n\r\t");  /* Get real keyword after '386' etc */
        if(key == NULL)
          continue;
        }
     ...
     if(key[0] == '[')
       ...
       if(strcmpi(key+1, "AreaSelection]") == 0)
         ...
         {
         sprintf(msg, "Unknow section: %s (timkeys.cfg, line %d)", key, lineno);
         Message(msg, -1, 0, YES);
         }
       continue;
       }
     ...
     if( (keycode = GetKeyCode(key)) == -1 )
       {
       sprintf(msg, "Unknown key: %s (timkey.cfg, line %d)", key, lineno);
       Message(msg, -1, 0, YES);
       continue;
       }
     ...
     if(keycode < 0)
       ...
       if( scope != MENUSCOPE)
         {
         sprintf(msg, "Menu item in non-menu section! (timkey.cfg, line %d)", lineno);
         Message(msg, -1, 0, YES);
         continue;
         }
       else  // We have to parse the literal string to be used as menu entry
         {
         if(NumMenuEntries >= (MAXMENUENTRIES-1))
           {
           sprintf(msg, "Can't have more than %d menu entries!", MAXMENUENTRIES);
           Message(msg, -1, 0, YES);
           continue;
           }

         ReaderMenu[NumMenuEntries].where = keycode;  // Is now MENUJAM or similar.
         ReaderMenu[NumMenuEntries].desc  = GetMenuDescription(&value); // Value is updated

         if(ReaderMenu[NumMenuEntries].desc == NULL)
           {
           sprintf(msg, "Invalid menu entry in line %d (timkeys.cfg)!", lineno);
           Message(msg, -1, 0, YES);
           continue;
           }
         }
     ...
     if( (commandcode = GetCommandCode(value, lineno)) == 0 )
       {
       sprintf(msg, "Unknown command: %s (timkeys.cfg, line %d)", value, lineno);
       Message(msg, -1, 0, YES);
       continue;
       }
...
sword GetCommandCode(char *s, int lineno)
{
     ...
     if(i == 0)               // Command code returned.
       ...
       if(newmaclen == -1)     // If this is the first element
          ...
          if(newmaclen == 0)  // There is already one element
             AddToMacro(&newmacro, &newmaclen, &macmaxlen, code);  // Add it.
          AddToMacro(&newmacro, &newmaclen, &macmaxlen, newcode);  // And the new one.
          }
       }
     else                     // Literal string -------------------------
       {
       ...
       while(*s && *s != quotechar)
         {
         AddToMacro(&newmacro, &newmaclen, &macmaxlen, (sword)*s);  // Add it.
         s++;
         }

       if(*s != quotechar)   // We didn't end with a quote?
          {
          sprintf(msg, "String without ending quote (timkeys.cfg, line %d)", lineno);
          Message(msg, -1, 0, YES);
          break;
          }
       else
   ...
   if(newmaclen > 0)    // This is indeed a macro. Work on it.
     {
     code = 513 + MakeNewMacro(newmacro, newmaclen);  // First macro == 513
     mem_free(newmacro);
     }
...
char * GetMenuDescription(char **charptr)
{
   ...
   *charptr = s;           // Update passed ptr for higher level f'ion
