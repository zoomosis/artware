...
static int pos = 0, row = 1;    /* pos == position on line (zero based */
                         /* row == row on screen, start with 1 (status
                            bar) */
...
static char templine[MAX_SCREEN_WIDTH]; /* 1 line buffer to manipulate
                                           throughout */
               ...
               int curtxtline)
{
    ...
    if (begin == NULL)          /* No text given, make first line
                                   ourselves */
    {
...
//           }
        ...
        case cEDITeditheader:  /* ALT-E */
            ...
            EditHeader(areahandle,
                       curmsg,
                       area->type == NETMAIL,
                       custom.aka, area->type == NETMAIL, area);
            SetEditCursor();
            putscreen();
            clockon();
            MakeTopLine(area, curmsg);
...
#endif
                if (!confirm
                    ("Do you really want to dump this message? [y/N]"))
                {
                    SetEditCursor();
                    break;
                }
        ...
        case cEDITsave:        /* ALT - S */
            ...
            if ((cfg.usr.status & EDITSAVEMENU)
                && (ShowEditMenu(curmsg, 1) == -1))
            {
                SetEditCursor();
                break;
            }
            ...
            if (c > 31 && c < 256) /* A legal char */
            {
                if (pos < (maxx - 1)) /* so a character can be added.. */
                {
                    ...
                    else        /* insert */
                    {
                        ...
                        {
                            templine[strlen(templine)] = '\0'; /* Just be
                                                                  sure! */
                            memmove(templine + pos + 1, templine + pos,
                                    strlen(templine) - pos);
                            templine[pos++] = c;
                        }
...
void update_screen(int entire)
{
    ...
    for (thisone = start; thisone && lines < (maxy - 2);
         thisone = thisone->next, lines++)
    {
        if (thisone == current)
        {
            reached_current = 1;
        ...
        else if (reached_current || entire
                 || (thisone->next && (thisone->next == current)))
        {
            if (thisone->status & HIGHLIGHT)
...
void add_n_wrap(char c, char insert)
{
    ...
    {
        unmarked = 1;
        unmark(NULL, 1);        // Shut off block, will be messed up
                                // anyway
    }
    ...
    if ((current->status & QUOTE)
        && (get_quote_string(current->ls, quotestring) != 0))
    {
        quote = 1;
    ...
    while (lptr)
    {
        ...
        if ((lptr->ls[0] != ' ') && (buffilled != 0) && (*(buffer + buffilled - 1) != ' ') && (!(lptr->status & NOSPACE))) /* Have 
                                                                                                                              to 
                                                                                                                              add 
                                                                                                                              a 
                                                                                                                              space? 
...
// !!!!!!!!!       current = lptr = fastFormatText(buffer, 65);
    ...
    if (quote)
    {
        ...
        {
            quotedline =
                mem_calloc(1, lptr->len + strlen(quotestring) + 1);
    ...
    if (prev)
        prev->next = current;   /* If this isn't first, make link with
                                   past :-) */
...
void import_file(void)
{
    ...
    getnamebox =
        initbox(9, 2, 14, 76, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(getnamebox);
    ...
    if (strlen(filename) > 69)
        filename[69] = '\0';
    if ((retval =
         getstring(12, 4, filename, 71, 71, VFNWW, cfg.col[Centry],
                   cfg.col[Cpoptext])) != 0)
    ...
    status =
        initbox(10, 25, 14, 55, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(status);
    ...
    while (fgets(temp, sizeof temp, infile) != NULL)
    {
        ...
        if (!(line++ % 10))
        {
            ...
            if (coreleft() < 10000L)
            {
                Message("Not enough memory to load entire file!", -1, 0,
                        YES);
                break;
            }
#endif
...
void press_enter(void)
{
    ...
    {
        if (pos == 0)           /* We add a blank line, make sure prev
                                   line is HCR ended */
        {
            if (current->prev)
        ...
        if (!(current->status & HCR)) /* Current line not HCR, so toggle
                                         off */
            lineptr->status &= ~HCR;
...
//         if(get_quote_string(current->ls, quotestring) != 0)
            ...
            {
                if (strncmpi(lineptr->ls, quotestring, strlen(quotestring))
                    != 0)
                {
                    tempstring =
                        mem_calloc(1,
                                   strlen(lineptr->ls) +
                                   strlen(quotestring) + 1);
                    if (oldpos < strlen(quotestring)) // Are we actually
                                                      // inside
                                                      // quotestring?
                    {
                        memset(quotestring, '\0', sizeof(quotestring)); // Don't 
                                                                        // quote 
                                                                        // quotes!
                    }
...
void delchar(void)
{
    ...
    else
    {
        ...
        {
            if ((span =
                 strcspn(current->next->ls,
                         " .!?")) < (maxx - strlen(templine) - 1))
            {
                add_n_wrap(0, 0);
                update_screen(0);
            }
...
void page_down(void)
{
    ...
    for (line = 1, walkline = first; walkline;
         walkline = walkline->next, line++)
    {
        if (walkline == current)
            break;
    }
...
void word_right(void)
{
    ...
    else
    {
        ...
        if ((*charptr == '\0') || (*(charptr + 1) == '\0')) /* Reached end 
                                                               of line,
                                                               start next
                                                               line */
        {
...
void unerase(void)
{
    ...
    enter_line();
    if (pos > strlen(templine))
        pos = strlen(templine); // Adjust pos on line.
...
void block_border(void)
{
    ...
    while (walkline && (walkline != current) && (walkline != bbegin))
...
int del_block(void)
{
    LINE *walkline, *bleft,     /* Begin of what is Left */
    ...
    *next;
    ...
    if (bbegin == first)        /* Delete first line? New begin = end of
                                   block */
    {
        if (bend)
    ...
    if ((bend == current) || ((!bend) && (bbegin == current))) /* Current
                                                                  line is
                                                                  deleted! 
                                                                */
        current = NULL;
...
void near statusline(void)
{
...
#ifndef __SENTER__
    sprintf(temp,
            " F1 = Help ³ Pos: %-3d Line: %-4d             %3luK           %s ",
            pos, line, ((unsigned long)coreleft()) / 1024,
            ((insert == 1) ? "Insert  " : "TypeOver"));
#else
    sprintf(temp,
            " %-3d,%-4d ³ ~F1~ Help   ~ALT-S~ Save   ~esc~ Abort   ~CTRL-Y~ Del line  %s ",
            pos, line, ((insert == 1) ? "Insert  " : "TypeOver"));
#endif
#else
    sprintf(temp,
            " F1 = Help ³ Pos: %-3d Line: %-4d                             %s ",
            pos, line, insert ? "Insert  " : "TypeOver");
#endif
    ...
    if (cfg.usr.status & CLOCK)
        biprint(maxy - 1, 0, cfg.col[Cmsgbar], cfg.col[Cmsgbaraccent],
                temp, '~');
    else
        biprinteol(maxy - 1, 0, cfg.col[Cmsgbar], cfg.col[Cmsgbaraccent],
                   temp, '~');
}
...
void MakeTopLine(AREA * area, MMSG * curmsg)
{
    ...
    sprintf(temp, "Area: %-15.15s To: %-0.36s (%-0.70s)", area->tag, to,
            subject);
    temp[maxx - 1] = '\0';
                 ...
                 MMSG * curmsg)
{
    ...
    getnamebox =
        initbox(9, 2, 14, 77, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
    drawbox(getnamebox);
    ...
    memset(filename, '\0', sizeof(filename));
    retval =
        getstring(12, 4, filename, 72, 72, "", cfg.col[Centry],
                  cfg.col[Cpoptext]);
    delbox(getnamebox);
    ...
    WhatToOpen =
        BuildCommandLine(filename, area, areahandle, curmsg, NULL, NULL);
    ...
    {
        inputbox =
            initbox(10, 15, 18, 65, cfg.col[Cpopframe], cfg.col[Cpoptext],
                    SINGLE, YES, ' ');
        drawbox(inputbox);
        boxwrite(inputbox, 1, 4, "File already exists! Append?");
        boxwrite(inputbox, 3, 4,
                 "[Y]es : Append to file (or press <ENTER>)");
