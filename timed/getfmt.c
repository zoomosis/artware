...
MMSG *GetFmtMsg(long curno, MSG * areahandle, AREA * area)
{
    ...
    unsigned len;
    char notforyou[] =
        "Sorry, this is a private message not from or addressed to you!";

    dword result = 0, txtlen = 0, nowread = 0, howmuch = 0, ctrllen = 0;
...
//    Message(msg, -1, 0, YES);
    ...
    if (level)                  // We need to translate chars, a charset
                                // kludge was present.
    {
    ...
    while (nowread < txtlen)
    {
        ...
        if (coreleft() < (5 * READBUFSIZE))
        {
            ...
            Message("Not enough memory! Message only partially read!", -1,
                    0, YES);
            break;
        }
#endif
        ...
        *(txtbuf + leftinbuf + (int)howmuch) = '\0'; // NULL terminate.
        ...
        // Fiddle with character translation. Don't forget there was still 
        // some
        // old junk in the buffer left, that was already translated. We do 
        // not
        ...
        if (level)              // We need to translate chars, a charset
                                // kludge was present.
        {
            newbuf = TranslateChars(txtbuf + leftinbuf, &len, level); // xlat 
                                                                      // new 
                                                                      // stuff
            if (newbuf)
            {
                ...
                memcpy(txtbuf, oldstuff, leftinbuf); // Old stuff from
                                                     // prev read
                memcpy(txtbuf + leftinbuf, newbuf, len); // The new
                                                         // translated
                                                         // stuff
                mem_free(oldstuff);
                mem_free(newbuf);
    ...
    if (IsPersonal(curmsg->mis.to, &curmsg->mis.destfido, area))
    {
        ...
        if (!(curmsg->mis.attr1 & aREAD))
        {
            if (MarkReceived(areahandle, area,
                             MsgGetCurMsg(areahandle), 0,
                             (curmsg->mis.attr1 & aCFM) ? 1 : 0) == -1)
            {
                Message("Error updating message to 'Received' status!", -1,
                        0, YES);
                showerror();
            }
        }
    ...
    {
        curmsg->ctext = CvtCtrlToKludge(curmsg->ctxt); /* Gives a
                                                          formatted copy,
                                                          original kept
                                                          for C)hange msg
                                                          for example */
        templptr = last = wraptext(curmsg->ctext, maxx, 1, 0); /* Format
                                                                  kludges */
        ...
        if (templptr)
            curmsg->txt = templptr; /* let it point to beginning of
                                       kludges, which is now message start 
                                     */
    }
...
MMSG *GetRawMsg(long curno, MSG * areahandle, int what, int convert)
{
    ...
    RAWBLOCK *current = NULL, *blk;

    char temp[256];
    char *tmp;
    int bodytoo = (what == READNOBODY) ? 0 : 1;
    sword level;
    dword result = 0, nowread = 0, howmuch = 0;
    ...
    if (bodytoo && curmsg->txtsize)
    {
        howmuch =
            (curmsg->txtsize >
             READBUFSIZE) ? READBUFSIZE : curmsg->txtsize;
        curmsg->firstblock =
            InitRawblock((unsigned)howmuch + 1, 1024, 16000);
    ...
    if (convert)                // Do we need to do conversion, or 'real
                                // raw' read?
        level = CheckForCharset(curmsg->ctxt);
    else
        level = 0;
    ...
    while (bodytoo && (nowread < curmsg->txtsize))
    {
        ...
        result =
            MsgReadMsg(msghandle, NULL, nowread, howmuch, current->txt, 0,
                       NULL);
    ...
    if (level)                  // If a CharSet kludge was there, remap to 
                                // local architecture.
    {
        TranslateHeader(&curmsg->mis, level);
...
LINE *wraptext(char *txt, int llen, int end, int savelast)
{
    ...
    while (*curptr != '\0')     /* Until end of text */
    {
        ...
        if (!lastsep && !(current->status & HCR))
        {
            ...
            buf[0] = '\0';
            pos = 0;
            curptr--;           // Go one position back, this isn't a
                                // space we can disregard!
        }
    ...
    if (!savelast && (!end || !pos)) // Shift rest back to begin of buffer 
                                     // for next pass..
    {
        memcpy(txt, buf, pos);
        txt[pos] = '\0';
        ...
        {
            if (current->prev)  // Only do this if there is a previous
                                // line
...
char *pascal near nextline(void)
{
    ...
    if (lastsep == ll && *curptr == 0x0D) // Ends in space, but followed
                                          // by HCR!
    {
        current->status |= HCR; // Indicate HCR
        curptr++;               // Skip this HCR, effectively added it to
                                // line now
    }

    // Now a situation where a word is separated by TWO spaces, and the
    // FIRST
    // space is the end of the line. This would result in the next line
    // starting
    // with the second space and looks bad. Strip the second space now, so 
    // it
    // won't happen.
...
void checklines(LINE * first)
{
    ...
    if (lastorigin)             /* Check for last origin in message */
    {
        ...
        if ((current->prev) && (strncmp(current->prev->ls, "---", 3) == 0)) /* Is 
                                                                               there 
                                                                               a 
                                                                               tearline? 
                                                                               Just 
                                                                             ...
                                                                             */
        {
            current->prev->status &= (NOSPACE | HCR); /* clear all bits
                                                         except TAB */
        ...
        for (current = current->next; current; current = current->next)
        {
            ...
            {
                current->status &= (NOSPACE | HCR); /* Clear all bits
                                                       except TAB */
    ...
    {
        current = lasttear;
        i = 0;                  // Count number of lines after possible
                                // tearline
        while (current->next)
        {
            i++;
        ...
        if (i < 3)              // Only if not more than 3 lines after
                                // tearline.
        {
            lasttear->status &= (NOSPACE | HCR); /* clear all bits except
                                                    TAB */
...
int IsPersonal(char *toname, NETADDR * address, AREA * area)
{
         ...
         l++)
    {
        ...
        {
            for (p = 0; ((cfg.usr.address[p].zone != 0) && (p < tMAXAKAS));
                 p++)
            {
