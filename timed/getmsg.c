...
{
    NETADDR afz;
    LINE *lptr = curmsg->txt, *lineptr, fakeline; // Used if a TO: line is 
                                                  // deleted..
    ...
    if (area->type != NETMAIL && area->type != MAIL)
        ...
        have_address = 1;       /* for netmail, the address is OK already */
    ...
    for (; lptr; lptr = lptr->next)
    {
        if (lptr->status & KLUDGE)
        {
            ...
            {
                if (curmsg->id[0] == '\0') /* Only if we don't have one -
                                              next might be MSGID in
                                              msgtext.. ?! */
                    strcpy(curmsg->id, lptr->ls + 8);
                if (area->type != NETMAIL && area->type != MAIL)
                {
                    tmpptr = curmsg->id;
                    while ((!(isdigit(*tmpptr))) && (*tmpptr != 0))
                        tmpptr++;
                    sscanf(tmpptr, "%hu:%hu/%hu.%hu", &afz.zone, &afz.net,
                           &afz.node, &afz.point);
            ...
            else if (strncmpi(lptr->ls, "\01REPLYTO", 8) == 0)
            {
                ...
                {
                    sscanf(tmpptr, "%hu:%hu/%hu.%hu",
                           &curmsg->rep_addr.zone, &curmsg->rep_addr.net,
            ...
            if (                /* FSC35 'replyto' kludges are both there */
                   ...
                   (curmsg->rep_gate[0] != '\0') &&
                   (curmsg->rep_name[0] != '\0'))
                curmsg->status |= REPLYTO; /* So remember this (and use in 
                                              ALT-N) */
        }
...
//              end--;
                ...
                while ((isdigit(*tmpptr) || *tmpptr == ':' || *tmpptr == '/') && (tmpptr > begin)) // Go 
                                                                                                   // tho 
                                                                                                   // start 
                                                                                                   // of 
                                                                                                   // address
...
void get_JAM_thread(MMSG * curmsg, MSG * areahandle)
{
    ...
    {
        if ((msghandle =
             MsgOpenMsg(areahandle, MOPEN_READ, nextread)) == NULL)
            break;
    ...
    if ((msghandle = MsgOpenMsg(areahandle, MOPEN_READ, current)) != NULL) /* Reset 
                                                                              current 
                                                                              MsgApi 
                                                                              msg 
                                                                            */
...
//     }
    ...
    if (!not &&
        ...
        (area->type == NETMAIL || area->type == LOCAL
         || area->type == MAIL))
    {
