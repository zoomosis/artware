                  ...
                  int hardcopy, int tagged, int what)
{
    ...
    {
        if ((outfile =
             OpenExist(cfg.usr.writename, area, areahandle,
                       curmsg)) == NULL)
            return;
    }
    else                        // Printer stuff..
                ...
                MMSG * curmsg)
{
    ...
    do
    {
        listreturn = 0;
        ret =
            getstring(13, 4, filename, 72, 72, "", cfg.col[Centry],
                      cfg.col[Cpoptext]);
        if (ret == TAB)
    ...
    if (area && areahandle && curmsg)
        WhatToOpen =
            BuildCommandLine(filename, area, areahandle, curmsg, NULL,
                             NULL);
    else
...
void do_print(FILE * outfile, AREA * area, MMSG * curmsg, int what)
{
    ...
    if (what == PRINTALL || what == PRINTHDR)
    {
        ...
        if (area->type == NETMAIL)
        {
            if (curmsg->mis.destinter[0] == '\0')
                ...
                sprintf(temp, "To   : %s (via %s, %s)\n",
                        curmsg->mis.destinter, curmsg->mis.to,
                        FormAddress(&curmsg->mis.destfido));
        }
        ...
        if (curmsg->mis.subj[0] == '\0'
            && (curmsg->mis.attr1 & (aFRQ | aFILE)))
        {
            SumAttachesRequests(&curmsg->mis, curmsg->mis.subj, 99,
                                SARboth);
        }
...
int ShowOutFiles(char *outstring)
{
    ...
    ret =
        pickone(flist, 5, (maxx / 2) - maxlen, maxy - 2,
                (maxx / 2) + maxlen);

    if (ret != -1)
    {
        for (i = 0, current = cfg.usr.outfiles; i < ret;
