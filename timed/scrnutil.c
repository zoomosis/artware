...
*/
    ...
    if (data->save == YES)      /* Should the screen "underneath" be
                                   saved? */
    {
        /* ozzmosis 2018-01-18 hack fix, replace "sizeof(unsigned short)"
           with "4" */
...
void savescreen()
{
    ...
    memcpy(scrptr->thisscreen, (char *)screen, len); /* Save it */
...
void putscreen()
{
    ...
    memcpy((char *)screen, temp->thisscreen, maxx * maxy * 2); /* Restore
                                                                  it */
...
void saveblock(int x1, int y1, int x2, int y2, char *b)
{
    ...
    asm push ds asm mov ax, y1  /* y1 */
     asm mov bx, maxx asm mul bl /* y1 * width of screen */
     asm add ax, x1 asm shl ax, 1 /* x1 * 2 */
     asm shl bx, 1              /* width * 2 */
     asm mov cx, x2 asm sub cx, x1 /* x2 - x1 */
     asm mov dx, y2 asm sub dx, y1 /* y2 - y1 */
     asm inc dx
        asm inc cx
        asm les di, b
        asm mov ds, vbase
        ...
        asm mov si, ax
        asm rep movsw
        asm add ax, bx asm dec dx asm jnz gr1 asm pop cx asm pop ds
#else
    xgettext(x1 + 1, y1 + 1, x2 + 1, y2 + 1, b);
#endif
...
{
#ifndef __WATCOMC__
    asm push ds asm mov ax, y   /* y */
     asm mov bx, maxx asm mul bl /* y * width */
     asm add ax, x              /* + x = base of block (upper left) */
     asm shl ax, 1              /* * 2 = no of bytes (screen = ints) */
     asm shl bx, 1              /* y * width in bytes (bytes 1 line) */
     asm mov es, vbase asm lds si, b /* b --> point to memblock to put on
                                        screen */
     asm mov dx, w              /* w */
     pr1:asm mov cx, dx         /* width? */
...
void drawrect(int x1, int y1, int x2, int y2, int style, int color)
{
    ...
    asm push ds asm mov es, vbase asm mov di, ofs asm mov ax, color asm mov cl, 8 asm shl ax, cl /* Put 
                                                                                                    color 
                                                                                                    byte 
                                                                                                    in 
                                                                                                    place 
    ...
    asm cld asm stosw asm mov cx, y2 asm sub cx, y1 asm dec cx /* Number
                                                                  of
                                                                  'horizontal 
                                                                  positions 
                                                                  for box
    ...
    asm rep stosw asm mov al, lines[1] /* Draw a '¿' */
    asm stosw asm mov cx, x2 asm sub cx, x1 /* Number of lines to draw
                                               ('³') */
     asm dec cx asm mov al, lines[5] /* Put a '³' in al */
    asm mov bx, maxx asm dec bx /* Adjust one pos down, for stosw inc's */
     asm dec bx                 /* And another one..  */
     asm shl bx, 1              /* Number of bytes on one line..  */
     asm sub bx, dx             /* .. minus width of box */
     loop_vert:asm add di, bx   /* Down one line */
    ...
    asm stosw asm mov al, lines[4] /* And a 'Ä' */
    asm mov cx, dx              /* Width of box */
     asm shr cx, 1              /* Make it number of words again */
     asm rep stosw asm mov al, lines[3] /* Put a 'Ù' in */
    asm stosw asm pop ds
#else
              ...
              unsigned short y2, char *s)
{
    ...
    result =
        WriteConsoleOutput(ScreenHandle, (CHAR_INFO *) s, coordsize,
                           coordloc, &writerect);
    if (result == FALSE)
              ...
              unsigned short y2, char *s)
{
    ...
    readrect.Left = x1 - 1;
    result =
        ReadConsoleOutput(ScreenHandle, (CHAR_INFO *) s, coordsize,
                          coordloc, &readrect);
