...
{
    int l, total, maxlen = 0, len, cur = 0, col, colhl, onescreen, start =
        0;
    BOX *pickbox;
    int key, found;

    for (total = 0;
         (choices[total] != NULL) && (choices[total][0] != '\0'); total++)
    {
    ...
    pickbox = initbox(y1, x1, y2, x2, COLFRAME, COLTEXT, SINGLE, YES, ' ');
    drawbox(pickbox);
    ...
    while (1)
    {
        ...
        for (l = start; l < (start + onescreen); l++) /* paint choices + 1 
                                                         highlighted */
        {
            ...
            if (l == cur)
            {
                ...
                if (help != NULL) // print help at bottom..
                    biprinteol(maxy - 1, 0, COLSTATTEXT, COLSTATHL,
                               help[l], ' ');
            }
