...
MMSG *ForwardMessage(MSG *toareahandle, AREA *toarea, AREA *area, MMSG *curmsg);
...
void CopyMarked(AREA *area, AREA *toarea, MSG *areahandle, MSG *toareahandle, int kill)
{
   ...
   for(i=0; i<area->mlist->active; i++)
     ...
     memset(&curmsg->mis.nextreply, '\0', sizeof(curmsg->mis.nextreply));

     result = SaveMessage(toareahandle,
                          toarea,
                          &curmsg->mis,
                          curmsg->firstblock,
                          curmsg->ctxt,
                          0,
                          -1);   // preserve == -1: no character translation!

     if(result == -1) stuffkey(27);  // Let's stop if there's an error!
...
void DeleteMarked(AREA *area, MSG *areahandle)
{
   ...
   for(i=0; i<area->mlist->active; i++)
     ...
     if(area->base & (MSGTYPE_SQUISH|MSGTYPE_HMB)) add++;
...

{
   ...
   border=initbox(10,19,14,60,cfg.col[Cpopframe],cfg.col[Cpoptext],SINGLE,YES,' ');
   drawbox(border);
...
//   clsw(cfg.usr..);
   ...
   if(c == 'f' || c == 'F')
     {
     message = ForwardMessage(toareahandle, toarea ? toarea : area, area, curmsg);
     if(!message)
        goto close_up;
     }
   else
     ...
     if(area->mlist->active != 0)  // We have Marked msgs!
       ...
       if(confirm("Process all marked messages? [y/N]") != 0)
         {
         if(toarea != NULL)
            CopyMarked(area, toarea, areahandle, toareahandle, (c=='m'||c=='M'));
         else
            Message("Mass Move/Copy to current area is not possible", -1, 0, YES);
...
//       }
     ...
     // Strip trailing junk.
     if(message->mis.seenby) FreeStringList(message->mis.seenby);
     if(message->mis.path) FreeStringList(message->mis.path);
     if(message->mis.via) FreeStringList(message->mis.via);
     ...
     memset(&message->mis.nextreply, '\0', sizeof(message->mis.nextreply));
     ...
     if(cfg.usr.status & SHOWNOTES)
        {
        sprintf(infotext, "* %s (from: %s) by %s using %s.\r\r", (c=='m' || c=='M') ? "Moved" : "Copied", area->tag, cfg.usr.name[custom.name].name, myname);
   ...
   if(c == 'f' || c == 'F')
     {
     check_cc(toarea ? toarea : area, toareahandle, &message->mis, &message->firstblock);

     if( (toarea && (toarea->type==NETMAIL)) || (!toarea && (area->type==NETMAIL)) )
       {
       /* Checked CC: first! Don't mess up zonegate etc!! */
       zonegate(&message->mis, message->ctxt, 0, toarea ? toarea->base : area->base);
       }
     }
...
MMSG *ForwardMessage(MSG *toareahandle, AREA *toarea, AREA *area, MMSG *curmsg)
{
   ...
   curtxtline=0;  // to update line pos in txt for later edit

   if(area->type == NETMAIL || area->type == ECHOMAIL)
     sprintf(from, "%s (%s)", &curmsg->mis.from, FormAddress(&curmsg->mis.origfido));
   else
   ...
   if(area->type == NETMAIL && curmsg->mis.destinter[0] != '\0')
     ...
     sprintf(to, "%s (%s)", &curmsg->mis.to, FormAddress(&curmsg->mis.destfido));
   else
     sprintf(to, "%s", &curmsg->mis.to);

   sprintf(infotext, "* Forwarded (from: %s) by %s using %s.\r* Originally from %s to %s.\r* Original dated: %s\r\r", area->tag, cfg.usr.name[custom.name].name, myname, from, to, MakeT(curmsg->mis.msgwritten, DATE_HDR));
