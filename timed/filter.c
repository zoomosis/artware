                   ...
                   int realbody)
{
    ...
    boxwrite(inputbox, 0, 1,
             "Give the name of the file to replace body with:");

    do
    {
        ret =
            getstring(13, 12, filename, 56, 79, "", cfg.col[Centry],
                      cfg.col[Cpoptext]);
    }
    while (ret != 0 && ret != ESC);
    ...
    if (realbody)               // We need to figure out what the
                                // tearline/origin is.
        origin = ExtractOrigin(curmsg);

    // We don't need the original body anymore, so let's free up the mem
    // used
    // by it.
    ...
    {
        if (origin)
            AddToBlock(curmsg->firstblock, origin, -1);
...
void FilterMemory(MMSG * curmsg, int realbody)
{
    ...
    if (origin)
    {
        ...
        else
        {
            ...
            if (curline->next)
                curline->next->prev = curline; // backward link.
        }
    }
...
char *ExtractOrigin(MMSG * curmsg)
{
    ...
    while (thisline)
    {
        ...
        {
            strcat(origin, "\r"); // Also for first line, make sure Orig
