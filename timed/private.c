...
int ScanMaySee(AREA * area, MSG * areahandle)
{
    ...
    {
        // Check this particular msg, if we may see it then add to the
        // list.
        if (MaySee
            (areahandle, MsgUidToMsgn(areahandle, uidtoscan, UID_EXACT)))
            AddMarkList(area->mayseelist, uidtoscan);

        // Find out the next message, using UID_NEXT and convert back to
        // uid
...
dword GetNextPrivate(AREA * area, MSG * areahandle)
{
    ...
    // If nothing came out of (shouldn't happen!), give some valid
    // number..
...
dword GetPrevPrivate(AREA * area, MSG * areahandle)
{
    ...
    // If nothing came out of it (shouldn't happen!), give some valid
    // number..
