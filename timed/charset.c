...
static CHARREC ReadRecBuffer[2],
               WriteRecBuffer[2];
...
int LoadCharMap(char *name, sword level, int readmap)
{
   ...
   if(readmap)
     ...
     for(i=0; i < UNKNOWNBUFFER; i++)
       {
       if(strcmpi(ReadUnknown[i], name) == 0 && ReadUnknownLevel[i] == level)
         return 0;
       }
...
//   Message("DiskRead for CharSet!", -1, 0, YES);
   ...
   sprintf(filename, "%s\\%smaps.dat", cfg.homedir, readmap ? "read" : "writ");
   ...
   while(fread(&CharRec, sizeof(CHARREC), 1, in) == 1)
     ...
     if(readmap == 1)
       ...
       if((strcmpi(CharRec.from_set, name) == 0) &&
         ...
         memcpy(&ReadRecBuffer[LastReadLoaded], &CharRec, sizeof(CHARREC));
         break;
         }
       }
     else  // writemap
       ...
       if(strcmpi(CharRec.to_set, name) == 0)
         ...
         memcpy(&WriteRecBuffer[LastWriteLoaded], &CharRec, sizeof(CHARREC));
         break;
         }
       }
     }
...
char * TranslateChars(char *s, unsigned *len, sword level)
{
   ...
   out = outptr = mem_malloc(*len + addedlen);  // addedlen for extra chars á = ss etc
   ...
   while(left)
       ...
       *outptr++ = *s++;
     ...
     else               // Extended char, translate. Or: level 1, so remap char
       {
       idx = *s;
       if(idx > 127)
       ...
       if(CharRec.lookup_table[idx][0] == 0 ||
         ...
         if(CharRec.lookup_table[idx][0] >= 32  &&
            ...
            {
            // We need to replace one char with two chars. First check if we
            // have enough extra space to expand it to two chars.
            if(nowadded >= (addedlen-2))  // extra also for trailing zero that we add!
              {
              pos = outptr - out; // Save position within output block,
                                  // realloc might move the block to another
                                  // memory location entirely!
              addedlen += 512;
              out = mem_realloc(out, *len + addedlen);
              outptr = out + pos;   // Set outptr back where we left off.
              }
         ...
         else  // Weird, strange, undefined or whatever, make it '?'
            {
            *outptr++ = '?';
            s++;
            }
...
char * TranslateBlocks(char *destset, RAWBLOCK *blk, char *kludges, MIS *mis)
{
   ...
   if(strcmpi(destset, "ASCII") != 0) // Do not add CHRS: ASCII, that's nonsense
      {
   ...
   if(strcmpi(destset, LOCALCHARSET) == 0)  // This is the same as local, so
...
void PickCharMap(void)
{
   ...
   if(ret != -1)
     {
     for(current=strlist, l=0; l<ret; l++, current=current->next)
       ;  /* nothing */
...
int IsLevelOne(char *s)
{
   ...
   for(current = cfg.FirstLevelOne; current; current = current->next)
     {
     if(strcmpi(current->s, s) == 0)   // Found it in list, so it's level 1!
        return 1;
     }

   return 0;  // If we got here, we didn't find it in the list. So it's level 2.
}
