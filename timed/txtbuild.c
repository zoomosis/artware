...
void AddToBlock(RAWBLOCK * blk, char *s, unsigned len)
{
    ...
    if ((blk->curlen + len + 1) > blk->curmaxlen) // Not big enough to
                                                  // add, mem_realloc
    {
        if (len > blk->delta)
            blk->delta = (len + 10); // Otherwise we trap with huge
                                     // additions..

        if ((blk->curmaxlen + blk->delta) > blk->maxlen) // Can't add to
                                                         // this one
        {
...
char GetLastChar(RAWBLOCK * blk)
{
    ...
    if (blk->curlen == 0)       // Nothing here, give last char of prev
                                // blk
...
void StripLastChar(RAWBLOCK * blk)
{
    ...
    if (blk->curlen == 0)       // Nothing here, strip last char of prev
                                // blk
    {
        *(prev->curend - 1) = '\0';
        prev->curlen--;
...
void SetLastChar(RAWBLOCK * blk, char c)
{
    ...
    if (blk->curlen == 0)       // Nothing here, goto last char of prev
                                // blk
        blk = prev;
...
RAWBLOCK *JoinLastBlocks(RAWBLOCK * first, int minsize)
{
    ...
    if ((tmp =
         realloc(prev->txt, last->curlen + prev->curlen + 1)) == NULL)
        return last;
