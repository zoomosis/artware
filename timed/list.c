...
void  readheader  (unsigned int num, MSG *areahandle, AREA *area, HDRLIST *thisone);
...
dword process_msgs(MSG *areahandle, AREA *area, int curpos, int operation);
...
dword MsgList(MSG *areahandle, AREA *area, dword current, char wide)
{
   ...
   listbox = initbox(0,0,rows+1,maxx-1,cfg.col[Casframe],cfg.col[Castext],SINGLE,NO,-1);
   ...
   while(curpos != -1)
      ...
      switch(get_idle_key(1, LISTSCOPE))
         ...
         case cLISTcopy:    /* ALT - C */
             ...
             if ( (current = process_msgs(areahandle, area, curpos, COPY)) == 0)
                {
         ...
         case cLISTmove:    /* ALT - M */
             ...
             if(area->mlist->active == 0)
              ...
              if ( (current = process_msgs(areahandle, area, curpos, MOVE)) == 0)
                 {
                 // Don't farfree(uidlist) here! Is already done!
                 return 0;
                 }
...
void readheader(unsigned num, MSG *areahandle, AREA *area, HDRLIST *thisone)

{
   ...
   thisone->line = mem_malloc(maxx+3);  // 1 for trailing '\0', 2 for ~xxx~
   ...
   if ((msghandle = MsgOpenMsg(areahandle, MOPEN_RDHDR, no)) == NULL)
      ...
      // Let's check for internet addresses, read body to get the TO: line
      ...
      if(BePrivate(area) &&
         ...
         strcpy(temp, "Personal message, not to/from you                                 ");
         }
      ...
      if(wide_display)
         ...
         if(thisone->status & PERSMSG)
           sprintf(thisone->line, "\xFF %5ld %-20.20s %s %s\xFF", no, mis.from, temp, MakeT(mis.msgwritten, DATE_LIST));
         else if(thisone->status & FROMMSG)
           sprintf(thisone->line, " %5ld \xFF%-20.20s\xFF %s %s", no, mis.from, temp, MakeT(mis.msgwritten, DATE_LIST));
         else
           sprintf(thisone->line, " %5ld %-20.20s %s %s", no, mis.from, temp, MakeT(mis.msgwritten, DATE_LIST));
         }
      else
         {
         temp[maxx-58]='\0';
         if(thisone->status & PERSMSG)
           sprintf(thisone->line, " %5ld %-20.20s \xFF%-20.20s\xFF %s %s", no, mis.from, mis.to, temp, MakeT(mis.msgwritten, DATE_LIST));
         else if(thisone->status & FROMMSG)
           sprintf(thisone->line, " %5ld \xFF%-20.20s\xFF %-20.20s %s %s", no, mis.from, mis.to, temp, MakeT(mis.msgwritten, DATE_LIST));
         else
           sprintf(thisone->line, " %5ld %-20.20s %-20.20s %s %s", no, mis.from, mis.to, temp, MakeT(mis.msgwritten, DATE_LIST));
         }
...
int fillstruct(MSG *areahandle, AREA *area, dword curmsgno)
{
   ...
   #ifndef __FLAT__
   uidlist = (dword huge *)farcalloc((unsigned long) MsgGetNumMsg(areahandle)+1, (unsigned long) sizeof(UMSGID));
   #else
   uidlist = calloc((size_t) MsgGetNumMsg(areahandle)+1, sizeof(UMSGID));
   #endif
   ...
   if(!(area->base & MSGTYPE_JAM))
     ...
     {
     wait = initbox(13, 33, 15, 47, cfg.col[Cpopframe], cfg.col[Cpoptext], SINGLE, YES, ' ');
     drawbox(wait);
...
int PrivateFillStruct(AREA *area, dword curmsgno)
{
   ...
   #ifndef __FLAT__
   uidlist = (dword huge *)farcalloc((unsigned long) area->nomsgs+1, (unsigned long) sizeof(UMSGID));
   #else
...
void checkfilled(int start, int rows, MSG *areahandle, AREA *area)
{
   ...
   if(!uidlist) Message("NULL UIDlist in listmode!", -1, 254, YES);
   ...
   for(n = start, l=0; l < rows; l++, n++)
      ...
      if(screennow)    // Lines already on screen (no need for reading hdr)?
        {
        for(i=0; i<(maxy-2); i++)
           {
           if(screennow[i].uid == newscreen[l].uid && screennow[i].line != NULL) // Yep! Copy info..
              {
   ...
   for(l=start, i=0; l < start+rows; i++, l++)
      ...
      if(curpos == l)
        ...
        if((doredraw) || (l==oldcur) || (curpos==l) )
          {
          biprint(l-start+1, 1, cfg.col[Cashigh], cfg.col[Cashighaccent], screennow[i].line, 0xFF);
        ...
        if((doredraw) || (l==oldcur) || (curpos==l) )
           {
           biprint(l-start+1, 1, cfg.col[Castext], cfg.col[Casspecial], screennow[i].line, 0xFF);
...

{
   ...
   CopyMarked(area, toarea, areahandle, toareahandle, (operation==MOVE) ? 1 : 0);
...
void print_msg(MSG *areahandle, AREA *area, long msgno, FILE *outfile)
{
   ...
   curmsg = GetFmtMsg(msgno, areahandle, area);       /* Read msg from disk, formatted */
...
void get_range(char dowhat, int maxpos, AREA *area)
{
   ...
   numbox = initbox(8,21,15,47,cfg.col[Cpopframe],cfg.col[Cpoptext],DOUBLE,YES,' ');
   drawbox(numbox);
   if(dowhat == TAG)
   ...
   while(action < 2)
      ...
      switch(action)
         ...
         case 0:
            ...
            while( (ret != ESC) && (ret != 0) )
               ret = getstring(12,40,nbegin,6,6,"0123456789",cfg.col[Centry], cfg.col[Cpoptext]);
            action = (ret == 0) ? 1 : 2;
            ret=1;
            break;
         ...
         case 1:
            ...
            while( (ret != ESC) && (ret != 0) && (ret != UP) && (ret != BTAB) )
               ret = getstring(13,40,nend,6,6,"0123456789",cfg.col[Centry], cfg.col[Cpoptext]);
...
void details(AREA *area, MSG *areahandle, int num, int start)
{
   ...
   if(thismsg->mis.subj[0] == '\0' && (thismsg->mis.attr1 & (aFRQ|aFILE)))
     {
   ...
   vprint(pos+1, 1, cfg.col[Cpoptext], " Date: %-31.31s %38.38s", MakeT(thismsg->mis.msgwritten, DATE_HDR), attr_to_txt(thismsg->mis.attr1, thismsg->mis.attr2));
   vprint(pos+2, 1, cfg.col[Cpoptext], " From: %-42.42s %-28.28s", thismsg->mis.from, FormAddress(&thismsg->mis.origfido));
   vprint(pos+3, 1, cfg.col[Cpoptext], " To  : %-42.42s %-28.28s", thismsg->mis.to, (area->type == NETMAIL) ? FormAddress(&thismsg->mis.destfido) : "");
   vprint(pos+4, 1, cfg.col[Cpoptext], " Subj: %-71.71s", thismsg->mis.subj);
...
void CheckInternet(MIS *mis, MSG *areahandle, dword no)
{
  ...
  if(strnicmp(body, "to:", 3) == 0)
    ...
    while(newline && i < 4)
      {
      while(*newline == 0x0A || *newline == 0x0D || *newline == 0x8D)
