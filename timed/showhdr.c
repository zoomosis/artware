...
void paint_header(MMSG * curmsg, word type)
{
    ...
    biprinteol(0, 0, cfg.col[Cmsgbar], cfg.col[Cmsgbaraccent],
               " ~F1~ Help  ~ins~ New msg  ~del~ Delete msg  ~R~ reply to msg  ~C~ Change msg  ~P~ Print",
               '~');

    printn(1, 0, cfg.col[Cmsgdata], "Date :", 6);
    vprint(1, 6, cfg.col[Cmsgdate], " %-32.32s",
           MakeT(mis->msgwritten, DATE_HDR));
    ...
    else
    {
        ...
        vprint(2, 50, cfg.col[Cmsgheader], "%-30.30s",
    ...
    else
    {
        ...
        if (type == NETMAIL)
            vprint(3, 50, cfg.col[Cmsgheader], "%-30.30s",
                   FormAddress(&mis->destfido));
    }

    vprint(1, 39, cfg.col[Cmsgattribs], "%41.41s",
...
void paint_header(MMSG * curmsg, word type)
{
    ...
    vprint(0, 6, cfg.col[Cmsgdate], " %-32.32s",
           MakeT(mis->msgwritten, DATE_HDR));
    ...
    else
    {
        ...
        vprint(1, 50, cfg.col[Cmsgheader], "%-30.30s",
               FormAddress(&mis->origfido));
    }
    ...
    vprint(0, 39, cfg.col[Cmsgattribs], "%41.41s",
           attr_to_txt(mis->attr1, mis->attr2));
...
char *FormAddress(NETADDR * address)
{
    ...
    if (address->point != 0)
        sprintf(a_string, "%hu:%hu/%hu.%hu", address->zone,
                address->net, address->node, address->point);
    else
...
char *MakeT(dword t, int type)
{
    ...
    {
    case DATE_LIST:
        sprintf(temp, "%s %2.2i", months_ab[tm->tm_mon], tm->tm_mday);
        break;

    case DATE_FULL:
        sprintf(temp, "%s %s %2.2i '%2.2i, %2.2i:%2.2i:%2.2i",
                weekday_ab[tm->tm_wday], months_ab[tm->tm_mon],
...
{
    static char temp[160];      /* Watch maximum length (# of possible
                                   attrs)!! */
    int l;
...
void MakeSubject(int x, int y, MIS * mis)
{
    ...
    if (!(mis->origbase & MSGTYPE_JAM) ||
        ((mis->origbase & MSGTYPE_JAM) && temp[0] == '\0'
         && !(mis->attached && mis->requested)))
    {
        if (mis->attached)
