...
void check_cc(AREA *area, MSG *areahandle, MIS *mis, RAWBLOCK **first)
{
   ...
   if(firsthdr)          // Only if we actually have any CC:'s to write!
...
void WriteCCs(MSG *areahandle, AREA *area, RAWBLOCK **first, MIS *mis)
{
   ...
   if(MoreToCome(firsthdr))   // Are there actually any unhidden names?
         ...
         if( (cfg.usr.status & CCVERBOSE) && (thisone->hidden == 0))
           ...
           if(area->type == NETMAIL || area->type == MAIL)
              ...
              if(thisone->mis->destinter[0] != '\0')
                  ...
                  sprintf(temp, "%s (%s)", thisone->mis->to, FormAddress(&thisone->mis->destfido));
              }
           else
     ...
     if(cfg.usr.status & CCVERBOSE)
        ...
        {
        sprintf(temp, "%d other recipient%s.\r", totalccs, totalccs > 1 ? "s" : "");
        AddToBlock(ccinfo, temp, -1);
        }
