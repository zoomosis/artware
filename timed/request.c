...
int showlist(REQLIST *first)
{
   ...
   filebox = initbox(5,maxx-17,rows+6,maxx-1,cfg.col[Casframe],cfg.col[Castext],SINGLE,YES,' ');
   drawbox(filebox);
   ...
   while(1)    /* Exit is w/ return statement that gives command back */
      ...
      switch (get_idle_key(1, GLOBALSCOPE))
         ...
         case 338:      /* <INS> */
            ...
            extrabox=initbox(13,24,17,56,cfg.col[Cpopframe],cfg.col[Cpoptext],SINGLE,YES,' ');
            drawbox(extrabox);
            print(15,26,cfg.col[Cpoptext],"Filename to add:");
            editret = getstring(15,43,extrafile,12,12,VFNWW,cfg.col[Centry], cfg.col[Cpoptext]);
            delbox(extrabox);
...
void writerequest(REQLIST *first, AREA *area, MSG *areahandle, MMSG *origmsg)
{
   ...
   while(curfile)
     {
     if(curfile->tagged)
       curmsg->mis.requested = AddToStringList(curmsg->mis.requested, curfile->name, NULL, 0);
...
//      retval=SetAttributes(header, 1);
   ...
   if(curmsg->mis.requested != NULL)
      writefa(&curmsg->mis, toarea ? toareahandle : areahandle, toarea ? toarea : area, 0);
   ...
   ScanArea(toarea ? toarea : area, toarea ? toareahandle : areahandle, toarea ? 1 : 0);
