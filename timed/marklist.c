...
void AddMarkList(MARKLIST * lst, dword no)
{
    ...
    {
        if ((tmp =
             mem_realloc(lst->list,
                         (lst->max + MLDELTA) * sizeof(dword))) == NULL)
            return;
        lst->list = tmp;
        lst->max += MLDELTA;
    }
    ...
    // Shift all one up..
    memmove(&lst->list[i + 1], &lst->list[i],
            ((lst->active - 1) - i) * sizeof(dword));
...
void RemoveMarkList(MARKLIST * lst, dword no)
{
    ...
    memmove(&lst->list[i], &lst->list[i + 1],
            (lst->active - i - 1) * sizeof(dword));
...
int IsMarked(MARKLIST * lst, dword no)
{
    ...
    if (bsearch(&no, lst->list, lst->active, sizeof(dword), (fptr) numeric)
        != 0)
        return 1;
...
dword NextMarked(MARKLIST * lst, dword start)
{
    ...
    ptr =
        (dword *) bsearch(&start, lst->list, lst->active, sizeof(dword),
                          (fptr) numeric);
