...
short int name_compare (char *keya, short int lena, char *keyb, short int lenb);
short int addr_compare (char *keya, short int lena, char *keyb, short int lenb);
...
void ConvertV7Flags(ADDRLIST *current, word node, char modem)
{
  ...
  for(i=0; i < MAXV7FLAGS; i++)
    ...
    if(match) // A match, so add this flag.
      {
      if((strlen(current->flags) + 1 + strlen(cfg.V7flags[i].flag)) < 60)
        {
        if(current->flags[0] != '\0') strcat(current->flags, ",");
