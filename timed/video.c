...
USHORT VioWrtCellStr(PCH pchCellString, USHORT cbCellString, USHORT usRow,
...
//{
    ...
    /* if it fails, it returns 0, so we set the screen to 25 lines,
       otherwise it returns the actual number of lines of text on the
       screen */
...
//}
    ...
    if (vbase == 0)
    {
        /* if you are in mode 7 the video is most likely based at 0xB000,
           otherwise it will be at 0xB800.  This fails for a small number
           of cards, like the genius mono displays (although, i've never
           had a failure reported... */
        ...
        /* if we are running desqview or topview, this function returns
           the segment of a video buffer to use, otherwise it returns the
...
// ============================= WIN NT/95 stuff ====================
    ...
    if (ScreenHandle == (HANDLE) ERROR_INVALID_HANDLE)
    {
        ...
        {
            fprintf(stderr,
                    "Error: Can't get screen handle with GetStdHandle().\n");
            return;
        }
    }
    ...
    {
        fprintf(stderr,
                "Error: Can't get screen buffer info with GetConsoleScreenBufferInfo().\n");
        return;
    }
             ...
             char sep)
{
    ...
    extern word slamlenbiline(unsigned char *d, char *s, short mask1,
                              short mask2, char sep);
#pragma aux slamlenbiline = \
    ...
    extern void slambiline(unsigned short *d, char *s, short mask1,
                           short mask2, char sep);
...
//     *out++ = ((attr << 8) | *line++);
    ...
    extern void slamleftbiline(unsigned short *d, char *s, short mask1,
                               short mask2, short left, char sep);
...
void print(int x, int y, int attr, unsigned char *line)
{
    ...
    asm push ds
        ...
        repeat:
        asm lodsb
        asm or al, al asm jz end asm stosw asm jmp repeat end: asm pop ds
#elif defined (__OS2__)
...
void printn(int x, int y, int attr, unsigned char *line, int len)
{
    ...
    asm push ds
        ...
        asm cld
        repeat:
        asm jcxz end asm lodsb asm stosw asm loop repeat end: asm pop ds
#elif defined (__OS2__)
    ...
    fResult = WriteConsoleOutputCharacter(ScreenHandle,
                                          line,
                                          len,
    ...
    WriteConsoleOutputAttribute(ScreenHandle,
                                vattrs, len, dwCursorPosition, &dwWritten);
...
void printeol(int x, int y, int attr, unsigned char *line)
{
    ...
    asm push ds
        ...
        asm loop again
        endstring:
        asm jcxz finito asm mov al, 32 asm rep stosw finito: asm pop ds
#elif defined (__OS2__)
    int len;
...
#ifndef __FLAT__
    extern void slamneol(unsigned short *d, char *s, int mask, int len,
                         int max);
#pragma aux slamneol  =   \
      "       cld          " \
      "       sub bx, cx   " \
...
void cls(void)
{
    ...
    asm push ds                 /* Save DS */
     asm mov cx, scrnsize
        asm mov es, vbase
        asm mov di, 0
        asm mov ah, 7 asm mov al, 32 asm cld asm rep stosw asm pop ds
#elif defined (__OS2__)
    ...
    FillConsoleOutputAttribute(ScreenHandle,
                               7, scrnsize, coord, &dwWritten);

    FillConsoleOutputCharacter(ScreenHandle,
                               0x20, scrnsize, coord, &dwWritten);
...
void clsw(unsigned char colour)
{
    ...
    asm push ds                 /* Save DS */
     asm mov cx, scrnsize
        asm mov es, vbase
        asm mov di, 0
        asm mov ah, colour asm mov al, 32 asm cld asm rep stosw asm pop ds
#elif defined (__OS2__)
...
//      USHORT cell = 0x0720;
    ...
    FillConsoleOutputAttribute(ScreenHandle,
                               colour, scrnsize, coord, &dwWritten);
                 ...
                 int attr, unsigned char token)
{
    ...
    asm push ds                 /* Save DS */
     asm mov es, vbase asm mov ax, y1 asm mov bx, x1 asm mov cx, maxx asm mul cl /* Begin 
                                                                                    regel 
                                                                                    * 
                                                                                    aantal 
                                                                                    kolommen 
     ...
     asm mov bx, x2 asm sub bx, x1 asm inc bx /* BX is aantal kolommen + 1 
                                               */
     asm mov dx, y2 asm sub dx, y1 asm inc dx /* DX is aantal rijen + 1 */
     asm mov ax, val asm cld loop: asm mov di, si /* Point to begin of 1
                                                     line in window */
     asm mov cx, bx             /* Aantal kolommen om op te werken */
     asm rep stosw              /* Fill this line */
     asm mov cx, maxx asm shl cx, 1 /* Aantal bytes op 1 regel */
...
/*    BYTE Cell[2] = {'*', 0x07); */
    ...
    for (i = y1; i <= y2; i++)
    {
        coord.Y = i;
        FillConsoleOutputAttribute(ScreenHandle,
                                   attr, x2 - x1 + 1, coord, &dwWritten);

        FillConsoleOutputCharacter(ScreenHandle,
                                   token, x2 - x1 + 1, coord, &dwWritten);
    }
                     ...
                     USHORT usColumn, USHORT hvio)
{
    ...
    if (fResult)
    {
        fResult = WriteConsoleOutputAttribute(ScreenHandle,
                                              vattrs,
                                              count,
                                              dwCursorPosition,
                                              &dwWritten);
