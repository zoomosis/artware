...
#define STACKSIZE 32767
    ...
    if (clockthreadid == -1)    // not started yet?
    {
        if (DosCreateMutexSem(NULL, &clocksem, 0L, FALSE) != 0)
            Message("Error creating clock semaphore (clocksem)!", -1, 0,
                    YES);

        if (DosCreateMutexSem(NULL, &activesem, 0L, FALSE) != 0)
            Message("Error creating clock semaphore (activesem)!", -1, 0,
                    YES);
        ...
        if ((clockthreadid =
             _beginthread(clockthread, NULL, STACKSIZE, NULL)) == -1)
...
//   if(!forced) DosSleep(1);
    ...
    if (showclock)
        VioWrtCharStrAtt(temp, (int)strlen(temp), maxy - 1, maxx - 8,
                         (PBYTE) & cfg.col[Cmsgbar], 0);
...
void killclock(void)
{
    ...
    if (clockthreadid != -1)
    {
        ...
        {
            sprintf(msg,
                    "Error releasing Clock semaphore (active, rc= %d)!",
                    rc);
            Message(msg, -1, 0, YES);
        }
