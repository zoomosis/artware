...
int picklist(char **choices, char **help, char *title, int y1, int x1, int y2, int x2)
{
   ...
   if(title)
     ...
     titlewin(pickbox, TLEFT, temp, cfg.col[Cpoptitle]);
     }
   ...
   while(1)
     ...
     for(l=start; l < (start+onescreen); l++)  /* paint choices + 1 highlighted */
       ...
       colhl = (l==cur) ? cfg.col[Cashighaccent] : cfg.col[Casaccent];
       if(l == cur)
          {
          MoveXY(x1+2, y1+2+l-start);
          if(help != NULL) // print help at bottom..
             biprinteol(maxy-1, 0, cfg.col[Cmsgbar], cfg.col[Cmsgbaraccent], help[l], '~');
          }
