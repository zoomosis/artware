...
void readconfig(void)
{
   ...
   cfg.usr.status |= (CONFIRMDELETE | NETTEAR | SHOWNOTES | ENDAREAMENU | CONFIRMEDITEXIT | LOWLEVELKB | JUMPYEDIT | CLOCK | JAMGETLAST | INTERNETEMPTY);
...
void analyse (char *line, int number)
{
   ...
   if(strcmpi(keyword, "386") == 0 ||
      strcmpi(keyword, "DOS") == 0 ||
      strcmpi(keyword, "OS2") == 0 )
      {
      #ifdef __OS2__
      ...
      keyword=strtok(NULL, " \n\r\t\0");  /* Get real keyword after '386' etc */
      }
   ...
   if(crc == crcV7MODEMTYPEBIT)
     ...
     V7Addflag(strtok(NULL, "\n"), NODEBIT);
      ...
      {
      sprintf(msg, "Unknown keyword: %-0.30s on line %d!", keyword, globalline);
      Message(msg, -1, 0, YES);
      }
}
...
void check_aka(AREA *current, char *rest)
{
   ...
   if ( ((charptr = strstr(rest, "-P")) != NULL) || ((charptr = strstr(rest, "-p")) != NULL) )
      ...
      if((aka=strtok(charptr, " \t\n\r")) != NULL)
         ...
         {
         sprintf(msg, "Can't figure out -P switch in line %d!", globalline);
         Message(msg, -1, 0, YES);
         return;
         }

      memset(&tempaddr, '\0', sizeof(NETADDR));
      sscanf(temp, "%hu:%hu/%hu.%hu", &tempaddr.zone, &tempaddr.net, &tempaddr.node, &tempaddr.point);
      ...
      if(n==(tMAXAKAS-1))
         {
         sprintf(msg, "Unknown AKA (%hu:%hu/%hu.%hu) referenced (line %d)!", tempaddr.zone, tempaddr.net, tempaddr.node, tempaddr.point, globalline);
         Message(msg, -1, 0, YES);
         }
...

{
   ...
   strncpy(temp, getquoted(value), 59);      /* First get the (quoted) description */
   temp[59] = 0;
      ...
      if ( ((charptr = strstr(rest, "-A")) != NULL) || ((charptr = strstr(rest, "-a")) != NULL) )
         ...
         while(*charptr != ' ' && *charptr != '-' && *charptr != '\t' && *charptr != '\n' && *charptr != '\0' && *charptr != '\r')
            ...
            switch(*charptr)
               ...
               default:
                    sprintf(msg, "Junk attribute in %s (line %d)", rest, globalline);
                    Message(msg, -1, 0, YES);
                    break;
               }
            charptr++;
...

{
   ...
   if( strchr(line, '"') == NULL || strchr(line, '"') == strrchr(line, '"') )
      {
      sprintf(msg, "Illegal line format in %0.30s (line %d)", line, globalline);
      Message(msg, -1, 0, YES);
      return temp;
      }
...

{
   ...
   switch(type)
     {
     case ECHOMAIL: thisarea->base |= MSGTYPE_ECHO ; break;
     case LOCAL   : thisarea->base |= MSGTYPE_LOCAL; break;
     case NETMAIL :
   ...
   if(!filename)
	...
	while( (line=xgetline(squishfile)) != NULL)
      ...
      if ( (keyword = strtok(line, " \t\r")) != NULL)
         ...
         else if((strcmpi(keyword, "LocalArea")==0) && (cfg.usr.status&READLOCAL))
            AddEchoArea(strtok(NULL, "\n"), LOCAL);
         else if((strcmpi(keyword, "BadArea")==0) && (cfg.usr.status&READLOCAL))
            AddEchoArea(strtok(NULL, "\n"), LOCAL);
          else if((strcmpi(keyword, "DupeArea")==0) && (cfg.usr.status&READLOCAL))
            AddEchoArea(strtok(NULL, "\n"), LOCAL);
          else if((strcmpi(keyword, "NetArea")==0) && (cfg.usr.status&READNET))
            AddEchoArea(strtok(NULL, "\n"), NETMAIL);
...
// ==============
     ...
     {
     areaoffset = (long) sizeof(CONFIG) + (long) fecfg->offset + (long) (fecfg->NodeCnt * sizeof(FE141Node));
     lseek(cfgfile, (long) areaoffset, SEEK_SET);

     FE141areas = mem_calloc(25, sizeof(FE141Area));  /* Read buffer to read disk */
     ...
     for(l=0, left = fecfg->AreaCnt; left > 0; left -= 25)
       ...
       if((got=read(cfgfile, FE141areas, (unsigned) (toread *sizeof(FE141Area)))) != (int) (toread * sizeof(FE141Area)) )
          {
          sprintf(msg, "Error reading %s! (got %d, wanted %d)", FEcfg, got, toread*sizeof(FE141Area));
          Message(msg, -1, 0, YES);
          close(cfgfile);
          mem_free(fecfg);
          mem_free(aka_array);
...
// ==============
   ...
   close(cfgfile);
   if(areas) mem_free(areas);
   if(FE141areas) mem_free(FE141areas);
   mem_free(aka_array);
...
void ReadNewFECFG(int cfgfile)
{
   ...
   areaoffset = (long) sizeof(CONFIG) + (long) fecfg->offset + (long) (fecfg->NodeCnt * fecfg->NodeRecSize);
   lseek(cfgfile, (long) areaoffset, SEEK_SET);

   FE142areas = mem_calloc(25, sizeof(FENEW_Area));  /* Read buffer to read disk */
   ...
   for(l=0, left = fecfg->AreaCnt; left > 0; left -= 25)
     ...
     if((got=read(cfgfile, FE142areas, (unsigned) (toread *sizeof(FENEW_Area)))) != (int) (toread * sizeof(FENEW_Area)) )
        {
        sprintf(msg, "Error reading %s! (got %d, wanted %d)", FEcfg, got, toread*sizeof(FENEW_Area));
        Message(msg, -1, 0, YES);
        close(cfgfile);
        mem_free(fecfg);
        mem_free(aka_array);
   ...
   close(cfgfile);
   if(areas) mem_free(areas);
   if(FE142areas) mem_free(FE142areas);
   mem_free(aka_array);
...
void ana_FE_area(Area *area, SysAddress *aka_array)
{
   ...
   sprintf(tempaka, "-P%hu:%hu/%hu.%hu", aka_array[area->flags.aka].main.zone,
...
void ana_FE142_area(FENEW_Area *area, FENEW_SysAddress *aka_array)
{
   ...
   if (area->flags.storage == FENEW_SQUISH)    /* Squish area */
        ...
        {
        Message("Unknown storage type found in Fastecho area!", -1, 0, YES);
        mem_free(thisarea);
        return;
        }
   ...
   sprintf(tempaka, "-P%hu:%hu/%hu.%hu", aka_array[area->info.aka].main.zone,
...
void addmacro(char *name, int usenet)
{
   ...
   if( (!macroptr) || (!toptr) )
      {
      sprintf(msg, "Illegal macro format in %s (line %d)", name, globalline);
      Message(msg, -1, 0, YES);
      return;
      }
...
int one_of_three(char *what)
{
  ...
  sprintf("Error: %s should be 'Yes', 'No' or 'Ask' (line %d)!", what, globalline);
  Message(msg, -1, 0, YES);
...
void ReadGEchoCFG(void)
{
   ...
   AREA *thisarea;
   int i,
   new = 0;      // New == newer than GEcho 1.02+
   ...
   {
      Message("This is an older, unsupported version of GEcho.", -1, 0, YES);
      mem_free(sys);
      return;
   }
...
*/
     ...
     if(!new)
       ...
       for(i=0; i<OLDAKAS; i++)
         ...
         if(sys->oldakaboard[i] != 0)
           ...
           sprintf(temp, "Netmail (%hu:%hu/%hu.%hu)", sys->oldaka[i].zone,
                                                      sys->oldaka[i].net,
                                                      sys->oldaka[i].node,
                                                      sys->oldaka[i].point);
   ...
   if(cfg.usr.status & READLOCAL)
     ...
     if(!new)
       ...
       if((sys->persmail_path[0] != '\0') || (sys->oldpersmailboard[0]!=0))
         {
         thisarea = init_area();
         ...
         thisarea->type = LOCAL;
         thisarea->tag = thisarea->desc = mem_strdup("Personal_Messages");
         AddLinkedArea(thisarea);
         }
       }
     else
...
*/
   ...
   arearecsize = AreaHdr.maxconnections * sizeof(CONNECTION) + AreaHdr.recsize;
   ...
   for (counter = 0; counter < records; counter++)
     ...
     if ((AreaFile.options & REMOVED) == 0)
        ana_GECHO_area(&AreaFile, new ? (GE_ADDRESS *)&sys->aka : (GE_ADDRESS *)&sys->oldaka);
     }

   close(AreaFileGE);

   // Now we check for the bad messages. If the boardnumber is not yet used,
   // we add it ourself. If it _is_ used, it was apparently already present
   ...
   if(cfg.usr.status & READLOCAL)
     ...
     if(new)
       {
       if(sys->badarea > 0 && sys->badarea < 201 && BoardNotUsed(sys->badarea))
         {
         thisarea = init_area();
         thisarea->base = MSGTYPE_HMB | MSGTYPE_LOCAL;
       ...
       if(sys->recoveryboard!=0 && BoardNotUsed(sys->recoveryboard))
         {
         thisarea = init_area();
         thisarea->base = MSGTYPE_HMB | MSGTYPE_LOCAL;
         itoa(sys->recoveryboard, temp, 10);
...
void ana_GECHO_area(AREAFILE_GE *area, GE_ADDRESS *aka_array)
{
   ...
   if(area->areaformat == FORMAT_PT || area->areaformat == FORMAT_PCB || area->areaformat > (NUM_FORMATS-1))
        return;
...
void ReadFmailCFG()
{
   ...
   if(isFmail120)
     ...
     memcpy(fmcfg, fm12cfg, oldcfgsize);  // We make a copy for the old stuff..
     }
   else
     {
   ...
   if(cfg.usr.status & READNET)
     ...
     for(i=0; i<FMMAX_AKAS; i++)
       ...
       if(fmcfg->netmailBoard[i] != 0)
         ...
         thisarea->type = NETMAIL;
         thisarea->stdattr |= aPRIVATE;
         sprintf(temp, "-P%hu:%hu/%hu.%hu", fmcfg->akaList[i].nodeNum.zone,
                                            fmcfg->akaList[i].nodeNum.net,
                                            fmcfg->akaList[i].nodeNum.node,
                                            fmcfg->akaList[i].nodeNum.point);

         check_aka(thisarea, temp);
         sprintf(temp, "Netmail (%hu:%hu/%hu.%hu)", fmcfg->akaList[i].nodeNum.zone,
                                                    fmcfg->akaList[i].nodeNum.net,
                                                    fmcfg->akaList[i].nodeNum.node,
                                                    fmcfg->akaList[i].nodeNum.point);
...
//       oldtype = 1;   Didin't work on .97 beta, identified as 0.94!
   ...
   if(strncmpi(hdr.versionString, "fmail", 5) != 0)   // Old type?
      ...
      while(n < hdr.totalRecords)
           {
           pos = (long)sizeof(headerType) + (long)((long)n*(long)hdr.recordSize);
...
void old_ana_Fmail_area(rawEchoTypeOld *area, nodeFakeType *aka_array)
{
   ...
   sprintf(tempaka, "-P%hu:%hu/%hu.%hu", aka_array[area->address].nodeNum.zone,
...
void ReadWtrCFG()
{
   ...
   if( (cfgfile=sopen(Wtrgatecfg, O_BINARY | O_RDONLY, SH_DENYNO)) == -1)
      {
...
void ana_wtr_area(WTRAREA *area, WTRAKA *akalist)
{
   ...
   if(area->deleted || area->fidoname[0] == '\0' || area->base == WTRPASSTHRU)
        return;
   ...
   sprintf(tempaka, "-P%hu:%hu/%hu.%hu", akalist[area->OriginAka-1].zone  ,
...
void AddLinkedArea(AREA *thisarea)
{
   ...
   while(curarea)
      ...
      if (strcmpi(curarea->tag, thisarea->tag) == 0)           /* Dupe! */
         {
         if( (curarea->base & MSGTYPE_SDM) && (thisarea->base & MSGTYPE_SQUISH))
           {
         ...
         if(thisarea->tag) mem_free(thisarea->tag);
         if(thisarea->dir) mem_free(thisarea->dir);
         mem_free(thisarea);
         return;
         }
      curarea = curarea->next;
   ...
   if((cfg.first == NULL) || (cfg.usr.alistsort[0] == '\0'))   // No sorting..
     {
...
int near filladdress(NETADDR *thisaddress, char *adr)
{
   ...
   if(*zone == '*') thisaddress->zone = (word) -99;
      else thisaddress->zone = (word) atoi(zone);

   if(*net == '*') thisaddress->net = (word) -99;
      else thisaddress->net = (word) atoi(net);

   if(*node == '*') thisaddress->node = (word) -99;
      else thisaddress->node = (word) atoi(node);

   if(point)
     {
     if(*point == '*') thisaddress->point = (word) -99;
...
void addwritename(char *filename)
{
   ...
   if(strlen(filename) > 119) filename[119] = '\0';
   ...
   if(cfg.usr.outfiles == NULL)
     ...
     {
     for(last = cfg.usr.outfiles; last->next != NULL; last = last->next)
       /* nothing */ ;       // Search last item, and..
...
void uucpaddress(char *address)
{
   sscanf(address, "%hu:%hu/%hu.%hu", &cfg.usr.uucpaddress.zone, &cfg.usr.uucpaddress.net, &cfg.usr.uucpaddress.node, &cfg.usr.uucpaddress.point);
}
      ...
      if ( (keyword = strtok(line, " \t\r")) != NULL)
         ...
         if(strcmpi(keyword, "MovePostedArticles")==0)
           ...
           if(tag && path && type)
             {
             thisarea = init_area();
             if(strchr(type, 'n') == NULL && strchr(type, 'N') == NULL)
               {
...
char *AllocRemapPath(char *s)
{
   ...
   if( (colon = strchr(s, ':')) != NULL && colon > s)  // > s, we need colon-1!
     {
...
void AddConvert(char *s)
{
   ...
   if( !s ||
     ...
     {
     Message("ConvertOutput statement without charset name!", -1, 0, YES);
     return;
     }

   if(strlen(charset) > (MAXCHARSETLEN-1)) charset[MAXCHARSETLEN-1] = '\0';
   strupr(charset);
...
void AddAssume(char *s)
{
   ...
   if( !s ||
     ...
     {
     Message("AssumeCharset statement without charset name!", -1, 0, YES);
     return;
     }
...
void ProcessCharsets(void)
{
   ...
   if(!CharSetList && !AssumeList) return;    // No conversions to process.
...
void V7Addflag(char *data, char type)
{
   ...
   if(!data)
     {
     sprintf(msg, "Incorrect V7 flag definition on line %d!", globalline);
     Message(msg, -1, 0, YES);
     return;
     }
...
void AddLevelOne(char *s)
{
   ...
   while(charptr)
     {
     strupr(charptr);
     cfg.FirstLevelOne = AddToStringList(cfg.FirstLevelOne, charptr, NULL, 0);
     charptr = strtok(NULL, " \t\r\n");
     }
