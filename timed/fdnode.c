...
static sword  near addmatch    (sword zone, sword net, sword node, sword point, long offset);
...
int    rawpoint = -1;
int    rawfdnet = -1;
int    foundamatch;  // When browsing, is starting point already found?
...
//        return 0;
   ...
   if(offset & 0x10000000L)       /* FDNET.PVT file */
      ...
      if(rawfdnet == -1)
         ...
         if( (rawfdnet = sopen(rawname, O_RDONLY | O_TEXT, SH_DENYNO)) == -1)
             {
      ...
      if(rawpoint == -1)
         ...
         if( (rawpoint = sopen(rawname, O_RDONLY | O_TEXT, SH_DENYNO)) == -1)
             {
             sprintf(msg, "Can't open %s!", rawname);
             Message(msg, -1, 0, YES);
             mem_free(current);
      ...
      if(rawnode == -1)
          ...
          if( (rawnode = sopen(rawname, O_RDONLY | O_TEXT, SH_DENYNO)) == -1)
             {
             sprintf(msg, "Can't open %s!", rawname);
             Message(msg, -1, 0, YES);
             mem_free(current);
