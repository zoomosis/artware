...
char ** dirlist(char *filespec, char tagging)
{
   ...
   while(1)
     ...
     printeol(0,0,cfg.col[Cmsgbar],temp);
     if(tagging)
        statusbar("Press <enter> or <space> to tag, ctrl-<enter> to accept.");
     else
     ...
     switch(ret)
       ...
       case 1:    // Change directory
         ...
         if(list->liststart[i]->status & FDIR)   // Go 'down' in directory
           {
   ...
   while(1)    /* Exit is w/ return statement that gives command back */
      ...
      for(l=start; l<start+rows; l++)
         ...
         if(!(curptr->status & (FDIR | UPDIR)))
            sprintf(temp, "  %-12.12s   %5d K ", curptr->name, curptr->size/1024);
         else
            sprintf(temp, "  %-12.12s           ", curptr->name, curptr->size/1024);
