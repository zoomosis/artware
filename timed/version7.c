...
ADDRLIST *ver7find(NETADDR * opus_addr)
{
    ...
    if (nodestart == 0)         /* Is root already known? Read ctl-block
                                   if not */
    {
...
ADDRLIST *opususer(char *name)
{
    ...
    *c = *p++;
    ...
    if (namestart == 0)         /* Is root already known? Read ctl-block
                                   if not */
    {
        ctl = mem_calloc(1, sizeof(struct _ndx));
        if (get7node(0, ctl) == -1)
...
{
    return (strnicmp
            ((char *)key, matchname, (unsigned int)min(namelen, len)));
}
...
void btree(int (*compare) (void *key, int len), long startblock,
           int onematch)
{
    int j, k, l;
    ...
    while (thisnode->ndx.INodeBlk.IndxFirst != -1)
    {
        ...
        for (j = 0; j < count; j++) /* check 20 or less */
        {
            ...
            if (k == 0)         /* Match right in the index */
            {
                ...
                {
                    if (j == 0)
                        btree(compare, thisnode->ndx.INodeBlk.IndxFirst,
                              onematch);
                    else
                        btree(compare,
                              (thisnode->ndx.INodeBlk.IndxRef[j - 1]).
                              IndxPtr, onematch);
                }

                /* Add this match to list */

                if ((addmatch(thisnode->ndx.INodeBlk.IndxRef[j].IndxData)
                     == -1) || (onematch != 0))
                {
                    mem_free(thisnode);
                    return;
 ...
 */
    ...
    if ((count = thisnode->ndx.LNodeBlk.IndxCnt) != 0)
    {
        ...
        for (j = 0; j < count; j++) /* check 30 or less */
        {
            ...
            {
                /* Add this match */
                if ((addmatch((thisnode->ndx.LNodeBlk.LeafRef[j]).KeyVal)
                     == -1) || (onematch != 0))
                    break;
            }
        }
...
int get7node(long blockno, struct _ndx *ndx)
{
    ...
    if (lseek(curindex, (long)blockno * 512L, SEEK_SET) !=
        (long)blockno * 512L)
    {
...
static int addmatch(long offset)
{
    ...
    for (current = first; current; current = current->next)
    {
        if (                    /* Check for dupe */
               ...
               (((vers7.NodeFlags & B_point)
                 && (current->address.point == vers7.HubNode))
                || ((!(vers7.NodeFlags & B_point))
                    && (current->address.point == 0))))
            return 0;
