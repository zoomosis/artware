...
void FindMessage(MSGA * areahandle, AREA * area, long last)
{
    ...
    baseframe =
        initbox(0, 0, maxy - 3, 79, cfg.col[Cfindactive],
                cfg.col[Cfindtext], DOUBLE, NO, ' ');
    drawbox(baseframe);
    delbox(baseframe);

    biprint(1, 21, cfg.col[Cfindtext], cfg.col[Cfindaccent],
            " ~timEd~ \07 Search configuration manager ", '~');
    ...
    {
        baseframe =
            initbox(0, 0, maxy - 3, 79, cfg.col[Cfindpassive],
                    cfg.col[Cfindtext], DOUBLE, YES, -1);
        drawbox(baseframe);
...
void ShowSearchOptions(int output)
{
    char areas[10], msgs[10], direction[12];
    char temp[MAX_SCREEN_WIDTH];
    ...
    if (output == 0)
    {
        printn(maxy - 5, 1, cfg.col[Cfindtext],
               " Areas :                 Messages:                 Direction:                 ",
               78);
...
int ShowSearchStrings(AREA * curarea)
{
    ...
    int howmany = 0, start = 0, curpos = 0, rows, colour;
    char temp[MAX_SCREEN_WIDTH];
    int i;
    BOX *baseframe;
  ...
  start:
    ...
    {
        bottom
            (" ~ENTER~ = edit   ~INS~ = add item   ~ALT-O~ = search Options   ~ctrl-ENTER~ = search");

        if (howmany > 0)
        {
            // find first to display
            for (i = 0, thisone = base.first; i < start;
                 i++, thisone = thisone->or)
                /* nothing */ ;
        ...
        case 280:              /* ALT - O */
            ...
            {
                sprintf(temp, "%-78.78s", MakeSearchString(highlighted));
                printn(3 + curpos - start, 1, cfg.col[Cfindtext], temp,
                       78);
            }
            GlobalOptions();
            delbox(baseframe);
        ...
        case 338:              /* Insert */
            ...
            howmany++;
            for (thisone = base.first; thisone && thisone->or != NULL;
                 thisone = thisone->or)
                /* nothing */ ;
            if (thisone)
        ...
        case 339:              /* Delete */
            ...
            {
                for (thisone = base.first;
                     thisone && thisone->or != highlighted;
                     thisone = thisone->or)
                    /* nothing */ ;
                ...
                if (thisone)    // Found
                {
                    ...
                    // rows--;
                    howmany--;
                    if (curpos > howmany - 1) // Don't point to
                                              // unexisting..
                        curpos--;

                    if (howmany >= (maxy - 9)) // If more than one screen
                                               // full..
                    {
                        if (start && (start > (howmany - maxy - 9))) // Last 
                                                                     // line 
                                                                     // still 
                                                                     // filled?
                            start--;
                        if (curpos >= (start + rows))
                            curpos--;
                    }
                }
                else
                    Message("Thisone not found in delete (main)!", -1, 0,
                            YES);
            }

            // Clean last line, there will be on less now (if screen not
            // full)
            if (howmany < (maxy - 9))
...
char *MakeSearchString(SEARCHARG * thisone)
{
    ...
    for (; thisone && (strlen(temp) < 78); thisone = thisone->and)
    {
        ...
        if (n < 4)
        {
            sprintf(tmpstring, "%-11.11s (%s)", thisone->string,
                    FullOptions(thisone));
...
void EditSearchArgument(SEARCHARG * first)
{
    ...
    elbox =
        initbox(topline, 21, topline + 14, 57, cfg.col[Cfindactive],
                cfg.col[Cfindtext], SINGLE, YES, ' ');
    drawbox(elbox);

    print(topline + 1, 22, cfg.col[Cfindtext],
          " String:                Options:");
    memset(temp, 'Ä', 35);
    temp[35] = '\0';
    ...
    {
        bottom
            (" ~ENTER~: edit item            ~INS~: add item            ~DEL~: delete item");
        if (howmany > 0)
        {
            // find first to display
            thisone = first;
            ...
            for (i = 0; i < rows; i++, thisone = thisone->and)
            {
                ...
                sprintf(temp, " %-20.20s   %s ", thisone->string,
                        FullOptions(thisone));
        ...
        case 338:              /* Insert */
            if (howmany < 10)
            {
                ...
                rows++;
                curpos = howmany - 1;
                for (thisone = first; thisone && thisone->and != NULL;
                     thisone = thisone->and)
                    /* nothing */ ;
                if (!thisone)
                {
        ...
        case 339:              /* Delete */
            ...
            if (highlighted == first)
            {
                ...
                {
                    for (mainline = base.first;
                         mainline && mainline->or != highlighted;
            ...
            {
                for (thisone = first;
                     thisone && thisone->and != highlighted;
                     thisone = thisone->and)
                    /* nothing */ ;
                if (thisone)    // Found
                {
            ...
            vprint(topline + 3 + howmany, 22, cfg.col[Cfindtext],
                   "%-35.35s", "");
            break;
...
void EditElement(SEARCHARG * el, int curline, int topline)
{
    ...
    do
    {
        ...
        ret =
            getstring(curline, 23, el->string, 20, 20, "", cfg.col[Centry],
                      cfg.col[Cfindtext]);
    }
    while (ret != 0 && ret != ESC);
...
//   print(curline, 23, cfg.col[Cfindtext], el->string);
    ...
    {
        elbox =
            initbox(topline, 21, topline + 14, 57, cfg.col[Cfindpassive],
                    cfg.col[Cfindtext], SINGLE, YES, -1);
        drawbox(elbox);
...
//     "   Search Origin line    ",
    ...
    obox =
        initbox(topline + 2, 27, topline + 12, 53, cfg.col[Cfindactive],
                cfg.col[Cfindtext], SINGLE, YES, -1);
    drawbox(obox);
...
//        options[5][1]='û';
        ...
        case 32:               /* space */
            ...
            case 2:
                ...
                break;
            case 3:
                thisone->where ^= wSUBJECT;
                break;
            case 4:
                thisone->where ^= wKLUDGES;
                break;
//             case 5:
//                thisone->where   ^= wORIGIN;  break;
            case 6:
                thisone->options ^= oWHOLE;
                break;
            case 7:
                thisone->options ^= oCASE;
                break;
            case 8:
                howmany = 0;
                if (thisone->where & wBODY)
                ...
                if ((howmany > 1) && (!(thisone->options & oNOT)))
                    Message
                        ("Can't combine NOT with more than one place to search (From, To etc.)",
                         -1, 0, YES);
                else
        ...
        case 27:               /* esc */
            if (thisone->where == 0)
            {
                sprintf(msg, "Please enter where to search for %s!",
                        thisone->string);
                Message(msg, -1, 0, YES);
            }
            else
...
void GlobalOptions(void)
{
    ...
    obox =
        initbox(topline, 21, topline + 5, 59, cfg.col[Cfindactive],
                cfg.col[Cfindtext], SINGLE, YES, ' ');
    drawbox(obox);
    ...
    while (1)
    {
        ...
        case 13:               /* enter */
            ...
            case 3:
                passive =
                    initbox(topline, 21, topline + 5, 59,
                            cfg.col[Cfindpassive], cfg.col[Cfindtext],
                            SINGLE, YES, -1);
                drawbox(passive);

                action =
                    picklist(Actions, NULL, topline - 1, 31, maxy - 1, 79);
                if (action != -1)
                {
                    // Clear action bits
                    base.action &= ~saALL;
                    ...
                    if (base.action & (saCOPY | saMOVE))
                    {
                        savescreen();
                        base.targetarea =
                            SelectArea(cfg.first, 1, cfg.first);
                        putscreen();
                        if (base.targetarea == NULL)
                        {
                            // No area given, back to read. (NULL ptr
                            // otherwise!)
                            base.action &=
                                ~(saREAD | saDELETE | saWRITE | saCOPY |
                                  saMOVE);
                            base.action |= saREAD;
                        }
                    }
                    ...
                    {
                        if ((out =
                             OpenExist(base.target, NULL, NULL,
                                       NULL)) != NULL)
                            fclose(out);
                        else
                        {
...
void DoSearch(MSGA * starthandle, AREA * startarea, long startlast)
{
    ...
    statusbox =
        initbox(maxy / 2 - 5, 24, maxy / 2 + 5, 56, cfg.col[Cfindactive],
    ...
    print(maxy / 2 + 1, 25, cfg.col[Cfindpassive],
    ...
    if (base.scope & sCURAREA)
    {
        ...
        if ((base.action & (saCOPY | saMOVE))
            && (base.targetarea == startarea))
        {
            Message("Target area of Copy/Move is current area!", -1, 0,
                    YES);
        }
        else
    ...
    // Not current area, so it is ALL or TAGGED
        ...
        while ((curarea = GetNextArea(curarea)) != NULL)
        {
            ShowSearchOptions(0);
            if ((base.action & (saCOPY | saMOVE))
                && (base.targetarea == curarea))
            {
            ...
            vprint(maxy / 2 + 2, 41, cfg.col[Cfinddata], "%-5lu     ",
                   ++totareas);
            vprint(maxy / 2 + 3, 41, cfg.col[Cfinddata], "%-5lu     ",
                   totmsgs);
...
int search_area(MSGA * areahandle, AREA * area)
{
    ...
    MoveXY(42, topline + 1);
    vprint(topline + 2, 41, cfg.col[Cfinddata], "%-5lu     ",
           area->highest);
    vprint(topline + 3, 41, cfg.col[Cfinddata], "%-5lu     ",
           area->lowest);
    ...
    while ((curno = GetNextMsg(curno, area, areahandle)) != 0)
    {
        ...
        maysee = 1;             // Are we allowed to see this msg (private 
                                // etc)? Start 'yes'.

        if (BePrivate(area) && (thismsg->mis.attr1 & aPRIVATE))
        {
            maysee = 0;         // Private area, start with 'on', unless
                                // this is from/to us.
            ...
            for (l = 0; (cfg.usr.name[l].name[0] != '\0') && (l < 10); l++)
            {
                if (strcmpi(thismsg->mis.to, cfg.usr.name[l].name) == 0)
                    maysee = 1;
                else if (strcmpi(thismsg->mis.from, cfg.usr.name[l].name)
                         == 0)
                    maysee = 1;
            }
        }
        ...
        {
            command =
                HandleMessage(curno, areahandle, area, thismsg, thisuid);
...
int check_arg(MMSG * thismsg, SEARCHARG * arg)
{
    ...
    {
        if ((retval =
             searchitem(thismsg->mis.from, -1, arg, ALWAYS, 1)) != -1)
            return retval;
    }
    ...
    {
        if ((retval =
             searchitem(thismsg->mis.to, -1, arg, ALWAYS, 1)) != -1)
            return retval;
    }
    ...
    {
        if ((retval =
             searchitem(thismsg->mis.subj, -1, arg, ALWAYS, 1)) != -1)
            return retval;
    }
    ...
    if (arg->where & wBODY)
    {
        for (thisblock = thismsg->firstblock; thisblock;
             thisblock = thisblock->next)
        {
            ...
            {
                // If there's a previous block, no match if it is first
                // word
            ...
            if ((retval =
                 searchitem(thisblock->txt, thisblock->curlen, arg,
                            howstrict, 0)) != -1)
                return retval;
        }

        if ((thisblock = thismsg->firstblock) != NULL) // If there are
                                                       // blocks at all
        {
            while (thisblock->next) // Is there (still) a transition to a
                                    // new block?
            {
                // Now we fill a buffer with last part or current block,
                // and first
                // part of next block, to see if any matches can be found
                // that are
                ...
                copylen1 =
                    (thisblock->curlen < 25) ? thisblock->curlen : 25;
                if (copylen1)
                    memcpy(tempbuffer,
                           thisblock->txt + (thisblock->curlen - 25),
                           copylen1);

                copylen2 =
                    (thisblock->next->curlen <
                     25) ? thisblock->next->curlen : 25;
                if (copylen2)
                    memcpy(tempbuffer + copylen1, thisblock->next->txt,
                           copylen2);
                ...
                if (thisblock->next->next == NULL) // end of buffer
                {
                    ...
                    {
                        // In that case the match may be the end of
                        // buffer!
                        howstrict &= ~NOTLAST;
                    }
                }
                ...
                if ((retval =
                     searchitem(tempbuffer, copylen1 + copylen2, arg,
                                howstrict, 0)) != -1)
                    return retval;
        ...
        // If we get here, we did not find the string in any of the
        // sub-strings.
        // If we have a 'NOT' search, that actually means we are
        // successful!
               ...
               char complete)
{
    ...
    {
        if ((complete != 0) && (arg->options & oNOT)) // Well, it's
                                                      // definitely not
                                                      // here.
            return 1;
        else
    ...
    if (bmgSearch(txt, len, &arg->pattern, howstrict) != NULL)
    {
        // Found a hit!
        if (arg->options & oNOT)
            return 0;
    ...
    else if (complete)          // Not found! If complete this is the
                                // string entirely,
    {                           // and we can be sure it's NOT there. Not
                                // so if this is only a part..
        if (arg->options & oNOT)
            return 1;
    }
                 ...
                 dword msgno, dword thisuid)
{
    ...
    SaveMessage(base.targethandle,
                base.targetarea,
                &curmsg->mis, curmsg->firstblock, curmsg->ctxt, 0, 0);
    ...
    for (curarg = base.first; curarg; curarg = curarg->or)
    {
        for (andarg = curarg; andarg; andarg = andarg->and)
        {
            if (andarg->options & oNOT)
                continue;       // Can't highlight all lines with NOT
                                // something..
            ...
            if (bmgSearch(line->ls,
                          line->len, &andarg->pattern, ALWAYS) == NULL)
                continue;
...
long GetOffset(dword crc, long *idxoffset, char *areatag)
{
    ...
    for (i = 0; i < howmany; i++)
    {
        if (index[i].crc == crc)
        {
            ...
            {
                Message("Invalid version number in find database!", -1, 0,
                        YES);
                goto ErrorExit;
            }
