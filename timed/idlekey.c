...
static int nt_kbhit(void)
{
    ...
    {
        fprintf(stderr,
                "Error: Unable to get input handle with GetStdHandle()\n");
        return 0;
    }
...
#if 0
            printf
                ("uc: %d  vk: %d  vs: %d  fShift: %d  fAlt: %d  fCtrl: %d\n",
...
asm mov ah, 11 h
        ...
        asm je go_on
        asm mov ah, 1 h
        go_on:asm int 16 h asm jnz goback asm mov ax, 0 goback:}
#endif
#endif
...
void check_enhanced(void)
{
    ...
    if ((ret = asmkbhit(enhanced)) != 0) // ZF is clear when key available
    {
        ...
        if (xkbhit())           // If no enhanced keyboard (no keystroke
                                // waiting),
