...
void showinfo(MMSG * curmsg, AREA * area, MSGA * areahandle)
{
...
#ifndef __OS2__
    infobox =
        initbox(0, 7, 24, 73, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
#else
    infobox =
        initbox(0, 7, 20, 73, cfg.col[Cpopframe], cfg.col[Cpoptext],
                SINGLE, YES, ' ');
#endif
    drawbox(infobox);
    ...
    vprint(3, 9, cfg.col[Cpoptext], "Date written : %s",
           MakeT(curmsg->mis.msgwritten, DATE_FULL));
    vprint(4, 9, cfg.col[Cpoptext], "Date arrived : %s",
           MakeT(curmsg->mis.msgprocessed, DATE_FULL));
    vprint(5, 9, cfg.col[Cpoptext], "Date received: %s",
           MakeT(curmsg->mis.msgreceived, DATE_FULL));

    vprint(6, 9, cfg.col[Cpoptext],
           "Message size : %lu (body: %lu  controlinfo: %lu)",
           (dword) (curmsg->txtsize + curmsg->ctrlsize), curmsg->txtsize,
           curmsg->ctrlsize);

    if (area->type == NETMAIL && (curmsg->mis.attr1 & (aFILE | aFRQ)))
    {
        vprint(7, 9, cfg.col[Cpoptext], "Files        : %-49.49s",
               SumAttaches(curmsg));
    }
    else
    ...
    vprint(15, 9, cfg.col[Cpoptext], "Attr : %s",
           attr_to_txt(area->stdattr, 0L));
    ...
    if (area->base & MSGTYPE_SDM)
        ...
        sprintf(temp,
                ...
                areadata->max_msg,
                areadata->skip_msg, areadata->keep_days);

    print(17, 9, cfg.col[Cpoptext], temp);

    vprint(18, 9, cfg.col[Cpoptext], "AKA  : %hu:%hu/%hu.%hu",
           cfg.usr.address[(int)area->aka].zone,
           cfg.usr.address[(int)area->aka].net,
           cfg.usr.address[(int)area->aka].node,
           cfg.usr.address[(int)area->aka].point);

    vprint(19, 9, cfg.col[Cpoptext], "Chars: %s (write), %s (read)",
           area->cswrite, area->csread);
    ...
    vprint(23, 9, cfg.col[Cpoptext], "Memory free : %dK",
...
//     print(23, 9, cfg.col[Cpoptext],temp);
    ...
    while (((key = get_idle_key(1, GLOBALSCOPE)) == 99) || (key == 67)
           || (key == 105))
    {
        if (key == 105)
        {
        ...
        while (action < 3)
        {
            vprint(17, 28, cfg.col[Cpoptext],
                   "%-4lu Skip=%-4lu Days=%-4u)                 ",
                   areadata->max_msg, areadata->skip_msg,
                   areadata->keep_days);
            ...
            case 0:
                ...
                while ((ret != ESC) && (ret != 0) && (ret != TAB))
                    ret =
                        getstring(17, 28, max, 4, 4, "0123456789",
            ...
            case 1:
                ...
                while ((ret != ESC) && (ret != 0) && (ret != BTAB)
                       && (ret != TAB))
                    ret =
                        getstring(17, 38, skip, 4, 4, "0123456789",
                                  cfg.col[Centry], cfg.col[Cpoptext]);
            ...
            case 2:
                ...
                while ((ret != ESC) && (ret != 0) && (ret != BTAB)
                       && (ret != TAB))
                    ret =
                        getstring(17, 48, keep, 4, 4, "0123456789",
                                  cfg.col[Centry], cfg.col[Cpoptext]);
...
char *SumAttaches(MMSG * curmsg)
{
    ...
    else if (curmsg->mis.attr1 & aFRQ) // Not JAM, reqs always in
                                       // subject...
    {
        strcpy(temp, "None");
        return temp;
...
void ShowFileInfo(MMSG * curmsg)
{
    ...
    for (curslist = curmsg->mis.attached; curslist;
         curslist = curslist->next)
        numattaches++;

    for (curslist = curmsg->mis.requested; curslist;
         curslist = curslist->next)
        numreqs++;
    ...
    biprint(top + 1, 8, cfg.col[Cfindactive], cfg.col[Cfindtext],
            "³ ~Attached files:~                     ³ ~Requested files:~      ³",
            '~');
    biprint(top + 2, 8, cfg.col[Cfindactive], cfg.col[Cfindtext],
            "³                                     ³ ~Filename     Password~ ³",
            '~');
    print(top + 3, 8, cfg.col[Cfindactive],
          "ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´");
    printc(top + height + 4, 46, cfg.col[Cfindactive], 'Ï');
    for (i = top + 4; i < top + height + 4; i++)
        printc(i, 46, cfg.col[Cfindactive], '³');

    for (current = alist, i = 0; current && i < height;
         current = current->next, i++)
    {
        vprint(top + 4 + i, 10, cfg.col[Cfindtext], "%-27.27s %4lu kB",
               current->name, current->size);
    }

    FreeAttachList(alist);

    for (curslist = curmsg->mis.requested, i = 0; curslist && i < height;
         curslist = curslist->next, i++)
    {
