...
void check_cc(AREA * area, MSG * areahandle, MIS * mis, RAWBLOCK ** first)
{
    ...
    if (doCCnow)                // Only if we actually have any CC:'s to
                                // write!
...
void WriteCCs(MSG * areahandle, AREA * area, RAWBLOCK ** first, MIS * mis)
{
    ...
    sprintf(temp, "%d other recipient%s.\r", doCCnow,
            doCCnow > 1 ? "s" : "");
    AddToBlock(ccinfo, temp, -1);
    ...
    *first = ccinfo;
    ...
    for (status = ReadFirstHeader(thisone); status == 0;
         status = ReadNextHeader(thisone))
    {
...
int ReadNextHeader(HDRLIST * hdr)
{
    ...
    for (i = 0; i < hdr->attaches; i++)
    {
        ...
        hdr->mis.attached =
            AddToStringList(hdr->mis.attached, &attfile, NULL, 0);
    }
    ...
    for (i = 0; i < hdr->requests; i++)
    {
        ...
        if (reqfile.password[0] == '\0') // !!!!!!!!
            hdr->mis.requested =
                AddToStringList(hdr->mis.requested, reqfile.filename, NULL,
                                0);
        else
            hdr->mis.requested =
                AddToStringList(hdr->mis.requested, reqfile.filename,
                                reqfile.password, 0);
    }
...
int WriteHeader(HDRLIST * hdr)
{
    ...
    for (thisone = hdr->mis.requested; thisone; thisone = thisone->next)
    {
        ...
        if (thisone->s)
            strncpy(reqfile.filename, thisone->s, 29);
        if (thisone->pw)
            strncpy(reqfile.password, thisone->pw, 29);
