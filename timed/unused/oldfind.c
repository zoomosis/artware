...
void FindMessage(MSG * areahandle, AREA * area, long last)
{
    ...
    if (!(REGISTERED))
        statusbar
            ("    Note: highlighting of lines with matches only in registered versions!");
    ...
    while (1)
    {
        ...
        for (fake = 0; (fake < 10) && (ARGS[fake].string[0] != '\0');
             fake++)
        {
            if ((ARGS[fake].where & FROM) ||
                (ARGS[fake].where & TO) || (ARGS[fake].where & SUBJECT))
                header = 1;
            if (ARGS[fake].where & BODY)
                body = 1;
        ...
        if (header || body)
            ...
            Message("I don't know where to search!", -1, 0, YES);
    }
...
int getss(AREA * area)
{
    ...
    while (loop)
    {
        if (!item)
            ret =
                getstring(8 + line, 37, ARGS[line].string, 20, "",
                          cfg.col[Centry]);
        else
        {
            MoveXY(60, 8 + line + 1);
...
/*                  select--;
        ...
        case RESET:
            ...
            for (fake = 0; (ARGS[fake].string[0] != '\0') && (fake < 10);
                 fake++)
            {
...
int searchloc(char line, AREA * area)
{
    ...
    while (loop)
    {
        ...
        case 328:              /* UP */
            ...
            break;
        case 336:              /* DOWN */
            ret = ENTER;        /* Don't tell anyone, same reaction as
                                   ENTER */
            loop = 0;
            break;
        case 315:              /* F1 */
...
void do_search(MSG * areahandle, AREA * area, long last)
{
    ...
    if (allareas)
    {
        for (thisarea = cfg.first; thisarea && !stop;
             thisarea = thisarea->next)
        {
            if (thisarea == area) /* Scanning the area we are in */
                curhandle = areahandle;
            else if ((curhandle =
                      MsgOpenArea(thisarea->dir, MSGAREA_CRIFNEC,
                                  thisarea->base)) == NULL)
            {
                sprintf(msg, "Can't open %s (%d)!", thisarea->tag,
                        heapcheck());
                Message(msg, -1, 0, YES);
                continue;       /* Can't open, skip to next */
            }

            vprint(16, 17, 7, "%-15.15s ", thisarea->desc);
            if (!thisarea->scanned)
                ScanArea(thisarea, curhandle);

            stop =
                search_area(curhandle, thisarea,
                            allmsgs ? 0L : thisarea->lr);

            if (thisarea != area) /* Don't close it if it's our 'current'
                                     area */
...
int search_area(MSG * areahandle, AREA * curarea, long start)
{
    ...
    vprint(17, 17, 7, "%4.4ld", start);

    for (msgno = start + 1;
         (msgno <= MsgGetHighMsg(areahandle)) && (command != ESC); msgno++)
    {
        if (xkbhit() && (get_idle_key(1) == 27))
            return 1;

        if ((msghandle =
             MsgOpenMsg(areahandle, MOPEN_READ, msgno)) == NULL)
            continue;

        if ((msgno % 10) == 0)
            vprint(17, 17, 7, "%5.5ld  ", msgno);
        ...
        else
        {
            ...
            if (txtlen)
                msgtext = mem_calloc(1, (unsigned)txtlen + 1);
            else
                msgtext = NULL;

            if (header)
                result =
                    MsgReadMsg(msghandle, &hdr, 0L, txtlen, msgtext, 0L,
                               NULL);
            else
                result =
                    MsgReadMsg(msghandle, NULL, 0L, txtlen, msgtext, 0L,
                               NULL);
        ...
        {
            if (msgtext)        /* mem_free some mem, we may need it! */
            {
                mem_free(msgtext);
                msgtext = NULL;
            }
            ...
            {
                foundmsg = GetFmtMsg(msgno, areahandle, curarea);
                mark_lines(foundmsg);
                clsw(cfg.col[Cmsgtext]);
                kbflush();
                if (curarea->mlist == NULL)
                    curarea->mlist = InitMarkList();
                command =
                    ShowMsg(foundmsg, curarea, areahandle, SCAN_DISPLAY);
                if (curarea->mlist->active == 0)
                {
                    DumpMarkList(curarea->mlist);
                    curarea->mlist = NULL;
                }
...
int check_msg(XMSG * hdr, char *msgtext, dword txtlen)
{
    ...
    for (loop = 0; loop < 10 && ARGS[loop].string[0] != '\0'; loop++)
    {

        if (ARGS[loop].where & FROM)
        {
            if ((bmgSearch
                 (hdr->from, strlen(hdr->from),
                  &ARGS[loop].pattern)) != NULL)
                return 1;
        }

        if (ARGS[loop].where & TO)
        {
            if ((bmgSearch(hdr->to, strlen(hdr->to), &ARGS[loop].pattern))
                != NULL)
                return 1;
        }
        ...
        {
            if (txtlen
                && (bmgSearch(msgtext, (int)txtlen, &ARGS[loop].pattern) !=
                    NULL))
                return 1;
        }
...
void load_search_data(void)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if (custom.sargs[i].string[strlen(custom.sargs[i].string) - 1] ==
            '\n')
            custom.sargs[i].string[strlen(custom.sargs[i].string) - 1] =
                '\0';
...
void save_search_data(AREA * area)
{
    ...
    if (!(REGISTERED))
    {
        Message("This function is only available in registered versions",
                -1, 0, YES);
        return;
    }
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        fprintf(data, "%-20.20s %-4.4s\n", ARGS[i].string,
                make_where(ARGS[i].where));
        sprintf(custom.sargs[i].string, "%-20.20s %-4.4s", ARGS[i].string,
                make_where(ARGS[i].where));
    }
