...
{                               // Used in the MSGIDX.BBS file
    word msgnum;
    byte board;
} idxrecord;

typedef struct hdrrecord
{                               // Used in the MSGHDR.BBS file
...
#define FMSGCRASH   0x0002      // accept for forwarding 0000 0000 0000
                                // 0010
#define FMSGREAD    0x0004      // read by addressee 0000 0000 0000 0100
#define FMSGSENT    0x0008      // sent OK (remote) 0000 0000 0000 1000
#define FMSGFILE    0x0010      // file attached to msg 0000 0000 0001
                                // 0000
...
#define FMSGXX1     0x0400      // STRIPPED by FidoNet<tm> 0000 0100 0000
                                // 0000
...
#define FMSGARQ     0x4000      // audit trail requested 0100 0000 0000
                                // 0000
...
#define CAP_STONAGE 0x0000      // No particular capabailties, Type 2
                                // assumed
...
typedef struct f2pktmsghdr
{
    word orignode,              // originating node
     ...
     year,                      // 1990 - nnnnn
     month, day, hour, minute, second, rate, // unused
     ver,                       // 2
     orignet,                   // originating net
     destnet;                   // destination net
     byte prod_lo,              // product code
     rev_major,                 // revision level, major
     password[8];               // packet level password
     word qm_origzone,          // QMail wants orig.zone here
     qm_destzone;               // QMail wants dest.zone here
     byte trash[2];             // junk[8]
     word cap_valid;            // Cap. validity, byte swapped
     byte prod_hi,              // product code, hi byte
     rev_minor;                 // revision level, minor
     word capability,           // bundling capability
    ...
    /* --- Note ----------------------------------------------------
       ...
       (*) In the original FSC-0001 specification, these fields were part
       of the Filler, and so may not be recognized by all software.
...
*/
    ...
    /* --- Note ----------------------------------------------------
       ...
       The last three fields are variable length; they are shown above for 
       reference, but are not part of the structure.
...
typedef struct eaddress
{                               // used in Area Manager ...
    struct naddress dstn;       // node number
    unsigned char exp_only:1;   // export only flag
    unsigned char imp_only:1;   // import only flag
    unsigned char paused:1;     // echo mail paused
    unsigned char rsvd1:5;      // reserved
} eaddress;

typedef struct fwd_link
{                               // used for forward request nodes ...
...
typedef struct im_config_type
{
    ...
    /* --- Note ----------------------------------------------------
       ...
       The information about the badmail-area, the dupe-area and the
       personalmail-area has been moved to IMAIL.AR, see there for
...
typedef struct areas_record_type
{
    ...
    byte brd;                   // board number
    char msg_path[80];          // MSG/Squish path
    word active:1;              // flag area active
    word zone_gate:1;           // Zone-gate stripping
    word tiny_seen:1;           // tiny seen-by flag
    word secure:1;              // secure flag
    word import_seen:1;         // import seen-by into base
    word deleted:1;             // flag deleted area
    word auto_added:1;          // flag auto-added record
    word mandatory:1;           // area is mandatory
    word read_only:1;           // area is read only
    word unlinked:1;            // area has been unlinked
    word ulnk_req:1;            // perform unlinked requests?
    word hidden:1;              // area is hidden
    word to_link:1;             // should by processed by LINK
    word check_dup:1;           // check for dupes in this area?
    word no_pause:1;            // %PAUSE not allowed in this echo?
    word hide_seen:1;           // Hide seens when importing
    word manual:1;              // No changes via Arealink
    word fwdreq_pending:1;      // Requested but yet not arrived
    word rsvd:6;                // reserved
    byte user_bits;             // 8 user-available bits
    byte days;                  // days to keep messages
    word msgs;                  // num messages to keep
    im_stats t_stats;           // TOSS stats
    ...
    /* --- Notes --------------------------------------------------
       ...
       1) The entries in 'o_addr' and 'use_akas' are indexes into the list 
       of AKAs in IMAIL.CF, minus 1. eg: im_config.aka[o_addr-1] A value
       of 0 means 'no address' 2) the 'user_bits' entry allows third-part
       software to store extra information in IMAIL.AR. Their meaning is
       program- specific, so be careful when making use of them!

       3) IMAIL.AR knows three predefined areatags:

       BADMAIL for the IMAIL badmail area, DUPES for the IMAIL dupe area
       and PERSMAIL for the IMAIL personal mail area.

       All three boards are treated as local areas and the BADMAIL area
       must be present and non-passthru otherwise IMAIL will not run.
