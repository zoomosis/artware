...
void MakeQuote      (MMSG *curmsg, AREA *area, word replytype, MIS *newmis, char *addtop);
...
void MakeMessage(MMSG *curmsg, AREA *area, MSG *areahandle, word reply, UMSGID reply_to_id, char *add_to_top)
{
   ...
   if(area->readonly)
     {
     if(reply && curmsg->ctxt && (GetCtrlToken(curmsg->ctxt, "AREA:") != NULL))
        {
   ...
   retval = MakeHeader(areahandle, curmsg, reply, area, reply_to_id, NULL, newmsg);
   ...
   if(!(newmsg->mis.attr1 & aFRQ) || confirm("Do you want to add a message body? (y/N)"))
   {
      if ((blk = GetBody(area, areahandle, newmsg, make_origin(custom.aka), firstmemline, 1, curtxtline)) == NULL)  /* Empty msg allowed for freq and attach */
         {
   ...
   if(SaveMessage(areahandle, area, &newmsg->mis, blk, kludges, 0, 0) == -1)
      goto close_up;
   ...
   if (reply && reply_to_id)     /* Write reply-info in msg we replied to */
      {
      if( (area->type==NETMAIL || area->type==MAIL) && (cfg.usr.delreply != 0) )
         {
         if(                                        /* 1 == no confirm */
             (cfg.usr.delreply == 1) ||
...
void ReplyOther(MMSG *curmsg, AREA *area)
{
   ...
   if(curmsg->ctxt && ((s=GetCtrlToken(curmsg->ctxt, "AREA:")) != NULL))
     {
...
void ChangeMessage(MMSG *curmsg, AREA *area, MSG *areahandle, int bodytoo)
{
   ...
   if((msghandle = MsgOpenMsg(areahandle, MOPEN_READ, MsgGetCurMsg(areahandle))) == NULL)
     {
   ...
   if( (curmsg->mis.attr1 & aSENT) || (curmsg->mis.attr1 & aSCANNED) )
       {
       if(confirm("Message already sent/scanned! Edit anyway? (y/N)") == 0)
          return;
       }
   ...
   if(bodytoo != -1)
     ...
     if (area->type == NETMAIL)
        editret = EditHeader(areahandle, curmsg, 1, custom.aka, 0, area);
     else
     ...
     while(((editret=SetAttributes(curmsg, area->base, 0)) != ESC) && (editret!=0))
         ;
   ...
   if(bodytoo == 1)
      ...
      /* We can't mem_free up mem here, might need original text if no body change.. */

      rawbody = GetBody(area, areahandle, curmsg, make_origin(custom.aka), firstmemline, 1, curtxtline);    /* could be NULL, then no TEXT write in MsgWrite Msg below.. */
   ...
   if(rawbody == NULL)
      {
      tempmsg = GetRawMsg(MsgUidToMsgn(areahandle, thisone, UID_EXACT), areahandle, READALL, 1);
      if(tempmsg)
        {
   ...
   if(curmsg->ctxt)
      ...
      if((newkludges=mem_realloc(newkludges, strlen(newkludges) + strlen(curmsg->ctxt)+256)) == NULL)
...
//      }
   ...
   if(replytype == 4)      // Bouncereply..
     ...
     if(custom.signoff[0] != '\0')
       {
       sprintf(temp, "\r%s",  expand(custom.signoff, &curmsg->mis, newmis));
       addit(temp, repfile);
       }
     ...
     sprintf(temp, "From: %s, (%s)\r",
                    curmsg->mis.from,
                    FormAddress(&curmsg->mis.origfido));  // static buffer!

     addit(temp, repfile);

     sprintf(temp, "To  : %s, (%s)\rSubj: %s\r--\r",
                   curmsg->mis.to,
                   FormAddress(&curmsg->mis.destfido),    // static buffer!
                   curmsg->mis.subj);
   ...
   if(custom.rephello[0] != '\0')
     {
     sprintf(temp, "%s\r\r", make_rephello(&curmsg->mis, newmis, replytype));
     addit(temp, repfile);
     }
...
//        blk = InitRawblock(200, 200, 64000u);
        ...
        while(1)               /* and append until end of paragraph */
           ...
           if(curline->ls) mem_free(curline->ls);
           curline->ls = NULL;
           mem_free(curline);
           ...
           if (curline->len < (maxx-20) )      /* short line, end of paragraph */
...
// !!!!!!!!!        lineptr = quote = wraptext(blk->txt, 60, 1, 0);  // 1 == also give last line back
        lineptr = quote = wraptext(blk->txt, 72 - strlen(inits) - 2, 1, 0);  // 1 == also give last line back
   ...
   curline = curmsg->txt;
	while(curline)
      ...
      if( !((curline->status & KLUDGE) && !parameterfile) )
        {
        if(curline->status & KLUDGE)  /* Don't put your own kludges in when change! */
          curline->ls[0] = '@';

        if(parameterfile && (curline->status & (ORIGIN|TEAR)))   // Invalidate first...
          {
          if(strncmpi(curline->ls, "---", 3) == 0)
             memset(curline->ls, '_', 3);
          else if(strncmpi(curline->ls, " * Origin: ", 11) == 0)
   ...
   *result++ = *name;
   ...
   if( ((tempptr=strchr(charptr, '@')) == NULL) &&
     ...
     while(*charptr)
        ...
        if(*charptr=='/')  // Stop! 'Malte Erikson / Beta tester' style
          break;
        ...
        if(*lineptr == '%')
          *tempptr++ = *lineptr++;
           ...
           if(mis->destinter[0] != '\0')
              {
              memcpy(tempptr, mis->destinter, strlen(mis->destinter));
           ...
           if(newmis->destinter[0] != '\0')
              {
              memcpy(tempptr, newmis->destinter, strlen(newmis->destinter));
           ...
           if(strchr(newmis->from, '@') == NULL)
             {
             memcpy(tempptr, newmis->from, strcspn(newmis->from, " "));
...
void zonegate(MIS *mis, char *kludges, int showinfo, dword base)
{
   ...
   if(mis->attr1 & mask)
     return;                 /* Don't gate crash or hold or frq/attach etc */

   if(mis->origfido.zone != mis->destfido.zone)  /* Inter zone msg! */
      {
      if(
          (cfg.usr.zonegate == 1) ||
          (gateconfirm(mis, showinfo) != 0)

        )
         {
         RemoveFromCtrl(kludges, "INTL");

         // If this is JAM, and we want to use the Zonegate bit, set it!
...
void add_text(char *line)
{
   ...
   if(firstmemline)   /* Only if we actually have previous lines */
     ...
     if(!(lastmemline->status & HCR))  /* No HCR? Then append */
       {
       charptr = mem_calloc(1, strlen(dupline) + lastmemline->len + 2);  // + 2 for trailing zero and possible added space */
       strcpy(charptr, lastmemline->ls); // start with previous line..

       /* Check if we need to add a space */
       if( (charptr[strlen(charptr)-1] != ' ') &&
           (dupline[0] != ' ') &&
           (!(lastmemline->status & NOSPACE)) )
           strcat(charptr, " ");
       ...
       if(firstmemline == lastmemline)  /* Are we changing/rewrapping the first line? */
          firstmemline = NULL;
       oldlast = lastmemline; // Save value, we need to mem_free() it later
       if(lastmemline->prev)
          {
   ...
   // wrap at a narrower margin, because we need to add the quotestring to
...
int WriteReplyLink(AREA *area, MSG *areahandle, dword orig, dword new)
{
  ...
  if(!(area->base & MSGTYPE_JAM))
    ...
    if((MsgReadMsg(msghandle, &replymis, 0, 0, NULL, 0, NULL)) != -1)
       ...
       for (n=0; n<9; n++)
         {
         if(replymis.replies[n] == 0)    /* Look for a mem_free 'slot' */
           {
  ...
  while(replymis.nextreply != 0) // Loop until space found..
    {
    if((msghandle = MsgOpenMsg(areahandle, MOPEN_RW, replymis.nextreply)) == NULL)
       return -1;
                                         ...
                                         char *kludges, dword no, int preserve)
{
   ...
   if(preserve == 1)  // We need to preserve the original. Make a copy and use that!
     {
     blk = MakeCopy(blk);
   ...
   if(mis && mis->destinter[0] != '\0')
     ...
     // Allow _no_ empty line between TO: and rest of body for those who need it.
     if(cfg.usr.status & INTERNETEMPTY)
        AddToBlock(tmpblk, "\r", 1);

     interblock = tmpblk;   // We use this later to free this block again.
   ...
   if(!(areahandle->locked))
      ...
      if(AttemptLock(areahandle) == -1)
         ...
         if(preserve == 1)
            {
            FreeBlocks(blk);   // Free our 'work' copy. Backup points to orig..
            FreeMIS(newmis);
            mem_free(newmis);
            }
         return -1;
   ...
   if(didlock) MsgUnlock(areahandle);
   ...
   else if(interblock != NULL) // We added internet address in front. Dump mem for that.
     {
...
int gateconfirm(MIS *mis, int showinfo)
{
  ...
  if(showinfo)
    {
    hdrinfo = initbox(1,1,3,78,cfg.col[Cpopframe],cfg.col[Cpoptext],SINGLE,YES,' ');
    drawbox(hdrinfo);
    sprintf(temp, "Message to: %s (%s)", mis->to, FormAddress(&mis->destfido));
    boxwrite(hdrinfo,0,0,temp);
    }

  retval = confirm("Send message through zonegate? (y/N)");

  if(showinfo) delbox(hdrinfo);
...
void RemoveTOline(RAWBLOCK *rawbody, MIS *mis)
{
   ...
   eol++;   // skip this CR
   if(*eol == '\r') eol++;   // also skip a next newline if it is there.
...
void AskDumpBody(RAWBLOCK *blk)
{
   ...
   if(confirm("Save message body to MSGTXT.TIM in timEd directory? (Y/n)"))
