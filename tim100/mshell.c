...
#else
    #define Mem_Tag_Err(a,b) mem_tag_err(a,__FILE__,__LINE__, b)
#endif
...
void Mem_Display (FILE *fp)
{
     ...
     fprintf(fp, "\nIndex    Size  File  -  Line - \n");
      ...
      #else
     fprintf(fp, "Index    Size - total size %lu bytes\n", mem_size);
      #endif
      idx= 0;
      p = memlist;
      while (p != NULL) {
  ...
  #else
     fprintf(fp, "Memory list not compiled. (define MEM_LIST)\n");
  #endif
}
...
void Mem_Check (FILE *fp)
{
 ...
 {
   fprintf(fp, "No errors detected, all mem de-allocated\n");
   Mem_Display(fp);
 }
}
