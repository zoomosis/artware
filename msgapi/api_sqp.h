...
dword  SquishReadMsg(MSGH *msgh,MIS *msg,dword offset,dword bytes,byte *text,dword clen,byte *ctxt);
sword  SquishWriteMsg(MSGH *msgh,word append,MIS *msg,byte *text,dword textlen,dword totlen,dword clen,byte *ctxt);
...
SQHDR * _SquishGotoMsg(MSG *sq,dword msgnum,FOFS *seek_frame,SQIDX *idx,word updptrs);
...
sword _SquishUpdateHeaderNext(MSG *sq,dword ofs,SQHDR *hdr,dword newval);
