...
static dword EXPENTRY SdmReadMsg(MSGH *msgh, XMSG *msg, dword offset, dword bytes, byte *text, dword clen, byte *ctxt)
{
  ...
  if (msg)
  {
    ...
    if(
        (!mi.useflags) &&
        ( (msg->attr & MSGCRASH) && (msg->attr & MSGHOLD) )
      )
       {
  ...
  {
    if ((text=fake_msgbuf=calloc(1,MAX_SDM_CLEN+1))==NULL)
    {
      msgapierr=MERR_NOMEM;
      return -1;
    }
  ...
  if (text)
  {
    ...
    got=farread(msgh->fd, text, (word)bytes);
  ...
  if (msg && msgh->ctrl)
    ConvertControlInfo(msgh->ctrl,orig,dest,msg);
...
static sword EXPENTRY SdmWriteMsg(MSGH *msgh, word append, XMSG *msg, byte *text, dword textlen, dword totlen, dword clen, byte *ctxt)
{
  ...
  if (msg)
  {
    ...
    {
      // We won't so this in echoes, nor if we're only updating [we might
      // overwrite the old klusges or body!!]
      if( (!msgh->sq->isecho) && (msgh->mode != MOPEN_RW) )
        {
        statfd=msgh->fd;
        msgh->zplen=WriteZPInfo(msg,WriteToFd,ctxt);
        }
    }
  }
  ...
  if (text)
    if (farwrite(msgh->fd,text,(word)textlen) != (sword)textlen)
    {
      msgapierr=MERR_NODS;
      return -1;
    }
...
/*    msgh->msg_len=(end < sizeof(XMSG)) ? 0L : end-(dword)sizeof(struct _omsg); */
    ...
    msgh->msg_len=(end < sizeof(struct _omsg)) ? 0L : end-(dword)sizeof(struct _omsg);
  ...
  if (msgh->clen == (dword)-1 && _Grab_Clen(msgh)==-1)
    ...
    return ((sdword) (msgh->msg_len-msgh->msgtxt_start) < (sdword)0L) ?
                                    0L : (sdword) (msgh->msg_len-msgh->msgtxt_start);
}
...
static sword near _SdmRescanArea(MSG *mh)
{
  ...
  if (findfirst(temp, &myffblk, 0) != -1)
  {
    ...
    do
    {
      ...
      if (myffblk.ff_fsize < sizeof(struct _omsg))
        continue;
      ...
      if ((thismsg=atoi(myffblk.ff_name)) != 0)
      {
...
int EXPENTRY WriteZPInfo(XMSG *msg,void (_stdc OS2LOADDS *wfunc)(byte *str),byte *kludges)
{
  ...
  byte *null="";
  int bytes=0;
  dword addmask = ADD_AS | ADD_DIR | ADD_IMM | ADD_KFS | ADD_TFS | ADD_LOK | ADD_CFM;
  ...
  if(msg->attr & addmask)
  {
    if(!mi.useflags) /* We can only simulate Direct :-( */
       {
       if(msg->attr & ADD_DIR)
          msg->attr |= (MSGHOLD | MSGCRASH);
       msg->attr &= ~addmask; /* Remove added attribs, *.MSG doesn't know
                                 them, they were added by me, remember! */
       }
    else
       {
