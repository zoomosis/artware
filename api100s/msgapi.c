...
static char *copyGvE=" JAM & Hudson specific MSGAPI extensions by Gerard van Essen (2:281/527), (c) 1993";
...
sword EXPENTRY MsgOpenApi(struct _minf *minf, char *path, word useflags, char *hudson)
{
  ...
  if(mi.hudsonpath[0] != '\0')
    if(HMBOpenBase() == -1)
       mi.hudsonpath[0] = '\0';  // Disable Hudson areas, can't open!
...
void EXPENTRY ConvertControlInfo(byte *ctrl,NETADDR *orig,NETADDR *dest, XMSG *hdr)
{
  ...
  if ((p=s=GetCtrlToken(ctrl, intl)) != NULL)
  {
    ...
    {
    orig->zone = dest->zone = norig.zone;
    }
  }   /* GvE: */
