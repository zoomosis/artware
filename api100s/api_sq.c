...
void convert_flags(XMSG *hdr, byte *ctxt);
                                  ...
                                  dword clen, byte *ctxt)
{
  ...
  switch (msgh->hdr->frame_type)
  {
    case FRAME_normal:
      ...
      if (bytes && text)
      {
        bytesread=(dword)farread(MsghSqd->sfd,text,(word)bytes);
...
static sword EXPENTRY SquishWriteMsg(MSGH *msgh, word append, XMSG *msg, byte *text, dword textlen, dword totlen, dword clen, byte *ctxt)
{
  ...
  if(msg && (clen || !append) )  /* Do we have to do it? */
    {
    if(msg->attr & addmask)
      {
      if(!mi.useflags) /* We can only simulate Direct :-( */
         ...
         newkludges = malloc(clen + strlen(temp));
         if(newkludges)
            {
            *newkludges = '\0';
            if(ctxt)
...
/**/
  ...
  if (msg)
  {
    ...
    oldattribs = msg->attr; /* Save this, not change for calling app! */
    msg->attr &= ~addmask; /* Remove added attribs, Squish doesn't know
  ...
  if (!append)
  {
    if ((ctxt && clen) || (clen && newkludges))
    {
      ...
      if(!newkludges)
         farwrite(Sqd->sfd, (char *)ctxt, (word)clen);
      else
         {
         farwrite(Sqd->sfd, (char *)newkludges, (word)clen);
         free(newkludges);
         newkludges = NULL;
         }
    }
  ...
  if (text)
  {
    ...
    if (howmuch)
      if (farwrite(Sqd->sfd, (char *)text, (word)howmuch) != (sword)howmuch)
      {
        msgapierr=MERR_NODS;
...
static sword near _SquishReadIndex(MSG *sq)
{
  ...
  if (ofslen &&
      (lseek(Sqd->ifd, 0L, SEEK_SET)==-1 ||
       farread(Sqd->ifd, (char far *)Sqd->idxbuf, (word)ofslen) !=
          (sword)ofslen))
  {
    farpfree(Sqd->idxbuf);
    Sqd->idxbuf=NULL;
...
void convert_flags(XMSG *hdr, byte *ctxt)
{
  ...
  if ((s=GetCtrlToken(ctxt,"FLAGS")) != NULL)
  {
