...
MSGA *SquishOpenArea(byte * name, word mode, word type)
{
    ...
    *sq->api = sq_funcs;
    ...
    {
        if (read(Sqd->sfd, (char *)&sqbase, sizeof sqbase) !=
            sizeof sqbase)
        {
...
sword SquishCloseMsg(MSGH * msgh)
{
    ...
    if (msgh->mode == MOPEN_CREATE)
    {
        ...
        lseek(MsghSqd->sfd,
              ...
              msgh->clen + (dword) msgh->bytes_written, SEEK_SET);


        if (msgh->trail != NULL)
        {
            if (write(MsghSqd->sfd, msgh->trail, msgh->trailsize + 1) ==
                -1)
                retval = -1;
        }
        else
                    ...
                    byte * text, dword clen, byte * ctxt)
{
    ...
    if (mis)
    {
        if (lseek(MsghSqd->sfd,
                  (long)((long)msgh->seek_frame +
                         (dword) MsghSqd->sz_sqhdr),
                  SEEK_SET) !=
        ...
        if (msgh->sq->sz_xmsg != sizeof(XMSG))
            lseek(MsghSqd->sfd,
                  (long)((long)msgh->sq->sz_xmsg - (long)sizeof(XMSG)),
                  SEEK_CUR);
    }
    ...
    case FRAME_normal:
        ...
        if (clen && ctxt)
        {
            if (lseek(MsghSqd->sfd,
                      (long)((long)msgh->seek_frame +
                             (long)MsghSqd->sz_sqhdr +
                             (long)msgh->sq->sz_xmsg), SEEK_SET) == -1L)
            {
                msgapierr = MERR_BADF;
                return -1;
            }
            ...
            if (clen < msgh->clen && bytes && text)
                lseek(MsghSqd->sfd, (long)((long)msgh->clen - (long)clen),
                      SEEK_CUR);
            ...
            bytesread = 0;
                       ...
                       (long)((long)msgh->seek_frame +
                              ...
                              (long)msgh->clen + (long)msgh->cur_pos),
                       SEEK_SET) == -1L)
        {
            msgapierr = MERR_BADF;
            return -1;
                     ...
                     dword textlen, dword totlen, dword clen, byte * ctxt)
{
    ...
    if ((sq->type & (MSGTYPE_NET | MSGTYPE_MAIL)) && mis && (clen || !append)) /* Do 
                                                                                  we 
                                                                                  have 
                                                                                  to 
                                                                                  do 
   ...
   **************************************************************************/
        ...
        if (msgh->msgnum == 0)
         ...
         /**/
            ...
            if (msgh->msgnum == sq->num_msg + 1 ||
                (ohdr =
                 _SquishGotoMsg(sq, msgh->msgnum, &oldofs, &here,
                                FALSE)) == NULL)
        {
            ...
            /* ...and the previous frame should be the former last
               message. */
            ...
            _SquishUpdateHeaderNext(sq, newhdr.prev_frame, &lhdr,
                                    this_frame);
        }
        else                    /* we're rewriting an 'old' message */
        {
            ...
            newhdr.next_frame = ohdr->next_frame; // We'll link in this
                                                  // frame in
            newhdr.prev_frame = ohdr->prev_frame; // the location of the
                                                  // old frame
...
// !!!!!         Add_To_Free_Chain(sq, oldofs, ohdr)==-1)
            ...
            _SquishUpdateHeaderNext(sq, newhdr.prev_frame, &save,
                                    this_frame);
            _SquishUpdateHeaderPrev(sq, newhdr.next_frame, &save,
                                    this_frame);
...
//  !!!!!      free(ohdr);
   ...
   **************************************************************************/
            /* If this was the first frame (ie. the first one pointed to
               by * sq->free_frame, which means that the first frame found 
               was long * enough to hold the message), then set the free
               pointer to the * start of the new free chain.  */
        ...
        /* Now update the linked list of free frames, to remove the
           current * frame from the free-frame list, if necessary.  We
           only need to do * this if the current frame wasn't just being
           appended to the .SQD * file, since there would be no links to
           update in that case.  */
...
// End GvE
    ...
    if (text)
    {
        ...
        if (howmuch)
            if (write(Sqd->sfd, (char *)text, (unsigned)howmuch) !=
                (int)howmuch)
            {
                msgapierr = MERR_NODS;
                return -1;
    ...
    if (!append)
    {
        /* Inc number of messages, and write into right spot in index
           file.  * (Frame location of msg #1 is stored at offset
           0*sizeof(dword) in * the index, msg #2's offset is stored at
...
//  }
   ...
   *************************************************************************/
    ...
    _SquishUpdateHeaderNext(sq, killhdr.prev_frame, &lhdr,
                            killhdr.next_frame);

    /* Fix the forward/next link */

    _SquishUpdateHeaderPrev(sq, killhdr.next_frame, &lhdr,
                            killhdr.prev_frame);
   ...
   *************************************************************************/
    ...
    if (sq->cur_msg == msgnum + 1) /* If we killed the msg we're on */
    {
        ...
        if ((hd =
             _SquishGotoMsg(sq, --sq->cur_msg, NULL, NULL, TRUE)) == NULL)
        {
...
dword SquishUidToMsgn(MSGA * sq, UMSGID umsgid, word type)
{
    ...
    {
        /* Scan each element, and if we find a match, return the right
           value */
...
//   char *endkludges = NULL;
    ...
    dword totlen = 0L,
        bodylen = 0L, clen = 0L, undeleted = 0L, lastframe = 0L;
    ...
    while (Sqd->free_frame && Sqd->free_frame != lastframe) // prevent
                                                            // endless
                                                            // loops.
    {
...
//     bodylen = sqheader.msg_length - sqheader.clen - sizeof(XMSG);
        ...
        if (read(Sqd->sfd, hdr, (unsigned)sizeof(XMSG)) !=
            (int)sizeof(XMSG))
        {
            msgapierr = MERR_READ;
            goto close_up;
        ...
        // Make sure it is NULL terminated. My routines will add a NULL if 
        // re-
        ...
        if (clen
            && ((long)(sqheader.frame_length - sizeof(XMSG) - clen)) <= 0)
        {
            clen = sqheader.frame_length - sizeof(XMSG); // No body?!
                                                         // Strange..
            bodylen = 0;
        }
        else
        {
            ...
            body = tottext;
            if ((long)
                (sqheader.frame_length - sizeof(XMSG) - clen - bodylen) <=
                0)
        ...
        if (SquishWriteMsg(msghandle,
                           0,
                           mis,
                           body, bodylen, bodylen, clen, kludges) == -1)
            goto close_up;
...
sword _OpenSquish(MSGA * sq, word * mode)
{
    ...
    if ((Sqd->sfd =
         sopen(temp,
               *mode == MSGAREA_CREATE ? fop_wpb : (O_RDWR | O_BINARY),
    ...
    if ((Sqd->ifd =
         sopen(temp, *mode == MSGAREA_CREATE ? fop_wpb : fop_rpb,
               SH_DENYNO, S_IREAD | S_IWRITE)) == -1)
    {
        if (*mode != MSGAREA_CRIFNEC)
        {
...
MSGH *_SquishOpenMsgRead(MSGA * sq, word mode, dword msgnum)
{
    ...
    else if (msgnum == MSGNUM_previous || msgnum == sq->cur_msg - 1L)
    {
        ...
        msgnum = MSGNUM_previous;
              ...
              _SquishGotoMsg(sq, msgnum, &seek_frame, NULL, TRUE)) == NULL)
        return NULL;
...
sword AddIndex(MSGA * sq, SQIDX * ix, dword num)
{
    ...
    if (num < Sqd->maxlen)      // Fits in alloced mem, or do we need
                                // realloc?
    {
    ...
    {
        tmpptr =
            (dword *) realloc(Sqd->uidlist,
                              (Sqd->maxlen + 100) * sizeof(dword));
...
sword Add_To_Free_Chain(MSGA * sq, FOFS killofs, SQHDR * killhdr)
{
    ...
    if (Sqd->free_frame == NULL_FRAME
        || Sqd->last_free_frame == NULL_FRAME)
    {
    ...
    else
    {
        ...
        if (_SquishUpdateHeaderNext
            (sq, Sqd->last_free_frame, &lhdr, killofs) == -1)
            return -1;
                      ...
                      FOFS * last_frame, SQHDR * lhdr, MSGH * msgh)
{
    ...
    *this_frame = Sqd->free_frame;
    ...
    /* Kill enough msgs to make room for this one, as long as there * is a 
       limit set, we're set up for dynamic renumbering, AND * we're
        ...
        else
        {
            ...
            *last_frame = NULL_FRAME;
                   ...
                   totlen + clen + (dword) sizeof(XMSG))
            {
                ...
                freehdr->frame_length +=
                    MsghSqd->sz_sqhdr + lhdr->frame_length;
...
#ifdef DEBUG
                fprintf(stdout,
                        "  Compact of %08lx and %08lx (new len=%ld)\n",
            ...
            if (freehdr->frame_length >=
                totlen + clen + (dword) sizeof(XMSG))
            {
                /* If one of te frames in our chain was the last free
                   frame, * set the last free frame to the one we've
                   merged it into, * for later a clean up effort.  */
...
//  }
    ...
    // and smaller, and speed will go up because there's not that much
    // shifting
    ...
    if (chsize
        (Sqd->ifd,
         (long)(((long)sq->num_msg - 1L) * (long)sizeof(SQIDX))) == -1)
        return -1;
    ...
    memcpy(Sqd->uidlist + msgnum - 1,
           Sqd->uidlist + msgnum, (sq->num_msg - msgnum) * sizeof(dword));
...
int SquishCalcTrailing(MSGH * msgh)
{
    ...
    if (lseek(MsghSqd->sfd, start, SEEK_SET) != start)
        return -1;
...
void SQ2MIS(MSGH * msgh, XMSG * xmsg, MIS * mis)
{
    ...
    if ((mis->attr1 & (aCRASH | aHOLD)) == (aCRASH | aHOLD)) // Direct for 
                                                             // squish!
    {
        if (mi.useflags == 0)   // We're set for a Squish environment
        {
            mis->attr1 |= aDIR; // Make it 'direct', and..
            mis->attr1 &= ~(aCRASH | aHOLD); // toggle off the Crash and
                                             // Hold.
        }
    }
    ...
    if (xmsg->attr & (MSGFRQ | MSGURQ)) // extract requested files from
                                        // subject.
