...
} API_HMBIDX;                   // Used to keep an 'index' of the area in
                                // memory
...
MSGA *HMBOpenArea(byte * name, word mode, word type)
{
    ...
    if (mi.hudsonpath[0] == '\0') // Is there a Hudson path? And open
        // files?
    {
...
//  debugdata = (HMBdata *)sq->apidata;
    ...
    if ((HMBDATA->board < 1) || (HMBDATA->board > 200))
    {
    ...
    /* If we are running in locked mode, (any area locked), we shouldn't
       read it (it might have been updated in memory, and the update would 
       then get lost!!!! */
...
MSGH *HMBOpenMsg(MSGA * sq, word mode, dword msgnum)
{
    ...
    if (msgnum != 0)            /* Not a new creation, get info */
    {
        ...
        if (mode != MOPEN_CREATE)
        {
            ...
            // Don't read kludges for MOPEN_RW, can't read/write them
            // anyway
            ...
            if ((mode != MOPEN_RW) && (mode != MOPEN_RDHDR))
            {
                ...
                // Conversion of attach/request is done here. We needed
                // the
                // kludges (for FRQ attribute!) first..

                if (msgh->mis.attr1 & (aFRQ | aURQ)) // extract requested
                    // files from
                    // subject.
                    Extract_Requests(&msgh->mis);

                if (msgh->mis.attr1 & aFILE) // extract attached files
                    // from subject.
    ...
    else                        /* New creation, msgnum = 0 */
    {
        ...
        /* If we only add one message, we can still make UID conversion
           work */

        if (sq->high_msg > HMBDATA->array_size) /* Space left to add? */
        {                       // We need to realloc, pity, may go wrong

            // Try to realloc, making space for 100 extra (300 bytes
            // extra)
            tmpptr = realloc(HMBDATA->msgs,
                             (HMBDATA->array_size +
                              100) * sizeof(API_HMBIDX));

            if (tmpptr == NULL) // Well, that failed, let MsgOpen fail
                // then
            {
                msgapierr = MERR_NOMEM;
                sq->high_msg--;
            ...
            HMBDATA->msgs = tmpptr; // Don't forget to use out newly
            // alloced array
        }
...
sword HMBCloseMsg(MSGH * msgh)
{
    ...
    if (msgh->mode == MOPEN_CREATE)
    {
        ...
        HMBWriteMsg(msgh, 1, NULL, msgh->trail, msgh->trailsize,
                    msgh->trailsize, 0L, NULL);
                 ...
                 dword clen, byte * ctxt)
{
    ...
    unsigned len;
    int moves = 0;
    int startlbytes = 0;        // How many Pascal length bytes to be
    // counted
    dword bytesread = 0L;
...
//    endlbytes = (msgh->rawclen + offset + bytes) / 256;
        ...
        start =
            (long)(((long)msgh->startblock * (long)sizeof(HMB_TXT)) +
                   (long)msgh->rawclen + 1L + (long)startlbytes +
                   (long)offset);
        ...
        /* Now we have to remove the length bytes from the Pascal
           strings.. */
        ...
        for (pos = 256; pos <= (msgh->cur_len + msgh->rawclen); pos += 256)
        {
            ...
            if ((pos - msgh->rawclen - offset) > bytes) /* Outside of msg
                                                           space */
                break;

            thischar = text + (pos - msgh->rawclen - offset - 1);
            len = (unsigned)((byte) thischar[0]); // How long is this
            // string
            if ((pos + len) > (msgh->rawclen + bytes + offset)) // Outside 
                                                                // 
                // of text 
                // read
                // this
        ...
        if (moves)
            *(text + bytes - moves) = '\0'; // If we moved text backwards, 
                                            // 
        // we have
        // room for this..
    }
                  ...
                  dword textlen, dword totlen, dword clen, byte * ctxt)
{
    ...
    if (!append)
    {
        ...
        if (write(HMBinfo.toidx, &toidx, sizeof(HMB_MSGTOIDX)) !=
            sizeof(HMB_MSGTOIDX))
        {
        ...
        if (msgh->numblocks == 1) /* Easy come, easy go.. */
        {
            ...
            if (write(HMBinfo.txt, txt, sizeof(HMB_TXT)) !=
                sizeof(HMB_TXT))
            {
                msgapierr = MERR_WRITE;
                return -1;
        ...
        {
            while (written < (textlen + msgh->rawclen)) // While there's
                // left to write..
            {
                memset(txt, '\0', sizeof(HMB_TXT));
                blen = curblen = 0;
                if (ctrlwritten < msgh->rawclen) /* We still need to write 
                                                    kludges */
                {
                    if ((blen = (msgh->rawclen - ctrlwritten)) > 255)
                        blen = 255;

                    memcpy(txt->str_txt, msgh->kludges + ctrlwritten,
                           blen);
                    ctrlwritten += blen;
                    written += blen;
                }
                ...
                {
                    if ((curblen =
                         (textlen + msgh->rawclen - written)) >
                        (255 - blen))
                        curblen = 255 - blen;
                    memcpy(txt->str_txt + blen,
                           text + (written - msgh->rawclen), curblen);
                    written += curblen;
                }
    ...
    else                        /* we're appending, resync to last block,
                                   fill her up, write rest */
    {
        if (textlen)
        {
            txt = calloc(1, sizeof(HMB_TXT));
            ...
            if (lseek(HMBinfo.txt,
                      (long)((long)
                             (msgh->startblock + msgh->blocks_written -
                              1L) * sizeof(HMB_TXT)), SEEK_SET) == -1)
            {
                msgapierr = MERR_SEEK;
                return -1;
            }

            if (read(HMBinfo.txt, txt, sizeof(HMB_TXT)) != sizeof(HMB_TXT))
            {
                msgapierr = MERR_READ;
                return -1;
            }
            ...
            {
                lseek(HMBinfo.txt, (long)(-1L * (long)sizeof(HMB_TXT)), SEEK_CUR); /* Back 
                                                                                      up, 
                                                                                      to 
                                                                                      rewrite 
                                                                                      this 
                ...
                if (txt->str_len + textlen > 255) /* Can't all put it in
                                                     this block */
            ...
            /* We filled out the last block of previous writes, now add
               rest in new blocks */
...
sword HMBKillMsg(MSGA * sq, dword msgnum)
{
    ...
    if (lseek
        (HMBinfo.toidx, (long)((long)recno * sizeof(HMB_MSGTOIDX)),
         SEEK_SET) == -1)
    {
        msgapierr = MERR_SEEK;
        goto Exit;
...
sword HMBLock(MSGA * sq)
{
    ...
    if (mi.haveshare)           // If not in Mtask env., no need to do
        // this..
    {
        if (HMBinfo.locks == 0) // First lock. Actually lock file & read
            // info
        {
...
UMSGID HMBMsgnToUid(MSGA * sq, dword msgnum)
{
    ...
    // high_msg should be array_size in case of multiple writes to the
    // area
    // 'in one lock', the in memory index won't be up-to-date in that
    // case!
    ...
    for (mn = 0; (dword) mn < sq->num_msg; mn++)
        if (HMBDATA->msgs[mn].msgno == wmsgid ||
            ...
            (type == UID_PREV && HMBDATA->msgs[mn].msgno <= wmsgid &&
             ((dword) (mn + 1) >= sq->num_msg
              || HMBDATA->msgs[mn + 1].msgno > wmsgid)))
            return ((dword) mn + 1);
...
int HMBOpenBase(void)
{
    ...
    HMBinfo.info = HMBinfo.idx = HMBinfo.toidx = HMBinfo.hdr = HMBinfo.txt = -1; // Set 
                                                                                 // 
    // to 
    // -1 
    // first, 
  ...
  ErrorExit:                   // At least one open failed, close all
    // opened files..
...
int near HMBScanArea(MSGA * sq)
{
    ...
    {
        if ((idx_array[l].board == testval)
            && (idx_array[l].msg_num != -1))
        {
...
void HMB2MIS(MIS * mis, HMB_MSGHDR * hmbhdr, MSGA * sq)
{
    ...
    memcpy(mis->from, hmbhdr->who_from + 1,
           (size_t) min(hmbhdr->who_from[0], 35));
    memcpy(mis->to, hmbhdr->who_to + 1,
           (size_t) min(hmbhdr->who_to[0], 35));
    memcpy(mis->subj, hmbhdr->subject + 1,
           (size_t) min(hmbhdr->subject[0], 71));
...
int near ReadKludges(struct _msgh *msgh)
{
    ...
    if (lseek
        (HMBinfo.txt, (long)((long)msgh->startblock * sizeof(HMB_TXT)),
         SEEK_SET) != (long)((long)msgh->startblock * sizeof(HMB_TXT)))
    {
        msgapierr = MERR_SEEK;
        return -1;
    }
...
int near HMBCalcTextLen(struct _msgh *msgh)
{
    ...
    *(txtptr + ((howmany - 1) * 256) + msgh->lastblocklen + 1) = '\0';
    ...
    // If this junk is not even longer than the rawclen, then we obviously 
    // 
    // have
    // a small message without any text. We then actually only have the
    // kludges
    // over here! (+5 : must at least be 'VIA <something>' or PATH:,
    // SEEN-BY)
    ...
    if (msgh->trailsize == 0)   // No SB, Path etc found. Only count
        // slack!
        msgh->trailsize = 255 - msgh->lastblocklen;
    else                        // We need to recalc to discount for
        // pascal strings
    {
...
#define HMBMASK2 (aHUB|aXMA|aHIR|aCOV|aSIG|aLET|aFAX)
    ...
    sprintf(temp, "%02d-%02d-%02d", jamtm->tm_mon + 1,
            jamtm->tm_mday, jamtm->tm_year % 100);

    hmbhdr->post_date[0] = (byte) 8;
    memcpy(hmbhdr->post_date + 1, temp, 8);


    sprintf(temp, "%02d:%02d", jamtm->tm_hour, jamtm->tm_min);
    ...
    if (msgh->mode != MOPEN_RW)
    {
        ...
        if (((mis->attr1 & HMBMASK1) || (mis->attr2 & HMBMASK2)) &&
            (sq->type & (MSGTYPE_NET | MSGTYPE_MAIL)))
        {
            Attr2Flags(temp, (mis->attr1 & HMBMASK1),
                       (mis->attr2 & HMBMASK2));
            strcat(msgh->kludges, temp);
        }
        ...
        if (((msgh->totlen + msgh->rawclen + msgh->trailsize) % 255) == 0)
            msgh->numblocks =
                (msgh->totlen + msgh->rawclen + msgh->trailsize) / 255;
        else
            msgh->numblocks =
                ((msgh->totlen + msgh->rawclen + msgh->trailsize) / 255) +
                1;
...
void near write_bbs_file(struct _msgh *msgh)
{
    ...
    if ((tosslog =
         sopen(temp, O_BINARY | O_CREAT | O_WRONLY, SH_DENYWR,
               S_IWRITE | S_IREAD)) == -1)
        return;
...
sword near HMBWriteIdx(sword msgnum, unsigned char board, sword recno)
{
    ...
    if (lseek(HMBinfo.idx,
              (long)((long)recno * sizeof(HMB_MSGIDX)), SEEK_SET) == -1)
    {
        msgapierr = MERR_SEEK;
        return -1;
    }
...
sword near HMBReadHeader(sword recno, HMB_MSGHDR * hdr)
{
    ...
    if (lseek
        (HMBinfo.hdr, (long)((long)recno * sizeof(HMB_MSGHDR)),
         SEEK_SET) != (long)((long)recno * sizeof(HMB_MSGHDR)))
    {
        msgapierr = MERR_SEEK;
        return -1;
    }
