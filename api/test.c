...
void main(void)
{
    ...
    for (current = 1; current <= high; current++)
    {
        ...
        if (MsgReadMsg(msghandle, &mis, 0L, bodylen, body, ctrllen, ctrl)
            == -1)
        {
...
//     printf("utc_ofs      : %d\n", mis.utc_ofs");
        ...
        printf("replies      : %d %d %d %d %d %d %d %d %d\n",
               mis.replies[0], mis.replies[1], mis.replies[2],
               mis.replies[3], mis.replies[4], mis.replies[5],
               mis.replies[6], mis.replies[7], mis.replies[8]);
        printf("nextreply    : %d\n", mis.nextreply);
        printf("origbase     : %d\n", mis.origbase);
        printf("msgno        : %d\n", mis.msgno);
        getchar();
        printf("requested    : ");
        PrintStringList(mis.requested);
        printf("attached     : ");
        PrintStringList(mis.attached);
        printf("seenby       : ");
        PrintStringList(mis.seenby);
        printf("path         : ");
        PrintStringList(mis.path);
        printf("via          : ");
        PrintStringList(mis.via);
        getchar();
        printf("extrasub     : %s\n",
...
char *AscAddress(NETADDR * addr)
{
    ...
    sprintf(address, "%u:%u/%u.%u", addr->zone, addr->net, addr->node,
            addr->point);
    return address;
}
